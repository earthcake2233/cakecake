...
> **不受应用上传链路影响**：本流程完全不经过应用的上传 API，也不使用 `uploads/`、`drafts/`、`raws/` 任何源文件命名空间——转码产物直接写到 `videos/{id}.mp4`、`covers/{id}.jpg`，数据库里写的是最终公网 URL 和状态。因此“直传 OSS 化、移除服务器端 `raws/` 上传”等改造都不会影响这条离线入料路径，它仍然是上传关闭时管理员发布视频的官方方式。
