...
6. 正常处理弹幕发送逻辑（S-013 颜色格式校验 → 敏感词过滤 → 入库、广播）。
...
**操作流程**：
请在根目录下的 `README_REFLECT.md`（架构与反思记录）中，追加一段格式如下的记录：
...
### S-014：编写 Mermaid 架构图

**对应 Rule**：R-DOC-11（架构图规则：流程用 Mermaid，项目结构用 Text）

**触发条件**：每次在 `.md` 文件中绘制架构图、拓扑图、流程图时，必须使用本 Skill。
**例外**：项目目录结构树（`├──` / `└──` 格式）不需要 Mermaid，使用纯文本 ` ``` ` 代码块即可。

**执行步骤**：

1. 选择图类型
   - `flowchart`（推荐）或 `graph` — 架构/拓扑/流程图
   - `sequenceDiagram` — 时序图
   - 优先使用 `flowchart`（语法更新，兼容性更好）
...
4. 子图（subgraph）注意事项
   - 标题含括号或中文时，必须用节点名+引号：
     ```
     subgraph Name["标题文本 (含括号)"]
     ```
   - 禁止子图内定义的节点在 `end` 后被引用（某些渲染器会报错）
...
6. 颜色和样式（保持默认，除非特别需要）
   - 用 `` style Node fill:#e74c3c,stroke:#333 `` 自定义
...
**常见错误**：
- 子图标题没加引号：`` subgraph ECS["标题"] `` 不是 `` subgraph ECS[标题] ``
- 数据库括号错位：`[("文本")]` 中 `)` 必须在 `"` 之后
- 跨子图引用节点：子图内定义的节点不要在 `end` 后使用

**验证方式**：
- 提交前在本地 Markdown 预览中确认渲染正常
- 若出现粉色/红色背景，说明语法有误，按本 Skill 逐行检查

### S-015：排查 Mermaid 粉色背景（渲染错误）

**对应 Rule**：R-DOC-11（架构图必须使用 Mermaid）

**触发条件**：在 Markdown 预览中看到 Mermaid 图表出现粉色/红色背景，说明语法有误。

**注意**：项目目录结构树（`├──` / `└──` 格式）不需要使用 Mermaid，用纯文本 ` ``` ` 代码块即可，不属于本 Skill 检查范围。

**常见语法错误排查步骤**：

1. **检查等号 `=`**
   - 节点文本避免出现 `=`（如 `status=ready`、`x=y`），某些渲染器会解释为属性赋值
   - ✅ 改为 `status ready` 或 `status  ready`
   - ❌ `L["DB: UPDATE SET status=ready"]` → ✅ `L["DB 更新状态为 ready"]`

2. **检查问号 `?`**
   - 菱形节点 `{}` 内避免使用 `?`
   - ❌ `O{"status ready?"}` → ✅ `O("status ready")` 改用圆角矩形

3. **检查子图跨域连线**
   - 子图（subgraph）内定义的节点，连线到另一个子图的节点时某些渲染器报错
   - ❌ `subgraph A / P --> Q / end` + `subgraph B / Q --> R`（跨子图引用）
   - ✅ 不使用跨子图连线，或将相关子图合并为扁平结构
...
5. **检查子图标题方括号**
   - 标题含括号或中文时，必须用 `Name["标题"]` 格式
   - ❌ `subgraph Name[标题]` → ✅ `subgraph Name["标题"]`

6. **检查中文特殊符号**
   - Mermaid 不支持节点文本中的 `→`、`←` 等箭头符号
   - ❌ `H["FFmpeg: 原始文件 → H.264"]` → ✅ `H["FFmpeg 转码 H.264"]`

7. **简化验证法**
   - 先建最小可渲染图（3-5 个节点，无子图无标签），确认渲染器正常
   - 逐步添加子图、标签、分支，每次添加后检查是否变粉
   - 最后一次变粉的步骤即为问题所在
