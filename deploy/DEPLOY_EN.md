...
**7 demo users are built in** (usernames are the video uploaders' nicknames): `暗猫の祝福`, `加载超时请稍后`, `Baka恶魔`, `三栗lili`, `Yeuoly`, `科学超电磁炮F`, `泛式大大` — all share password `demo123456` (overridable via `DEMO_USER_PASSWORD`); you can also register your own account. The admin console user is `admin`; the password comes from `ADMIN_SEED_PASSWORD` in `.env` (randomly generated by `scripts/init_env.py`, effective only on first init; admins have no password-change endpoint). All ports bind to the loopback interface by default; if 8888/8080 are already taken, override `WEB_PORT` / `BACKEND_PORT` in `.env` and run `docker compose up -d` again. For public deployment restrict port exposure and keep secrets safe.
...
> After enabling local monitoring, the nginx config already includes
> `location = /metrics` (Bearer-auth reverse proxy to `127.0.0.1:8080`). Set
> `METRICS_TOKEN` in the server `.env`; see
> [monitoring_EN.md](../docs/monitoring_EN.md#4-cloud-server-integration-production).
...
- Optional CI deploy: [.github/workflows/deploy.yml](../.github/workflows/deploy.yml) — every deploy merges the
  latest `deploy/env.production.example` into `/opt/minibili/.env`: existing keys (including secrets/tokens) are
  never overwritten, only new template keys are appended, and the previous file is backed up as `.env.prev`
  (see [scripts/merge_env.sh](../scripts/merge_env.sh)).
