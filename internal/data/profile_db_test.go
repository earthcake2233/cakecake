...
func TestProfileCount_Zero(t *testing.T) {
	...
	require.NoError(t, err)
	if n != 0 { t.Errorf("expected 0, got %d", n) }
}
...
func TestListAgentProfiles_Empty(t *testing.T) {
	...
	require.NoError(t, err)
	if len(list) != 0 { t.Errorf("expected 0, got %d", len(list)) }
}
...
func TestGetAgentProfile_Found(t *testing.T) {
	...
	require.NoError(t, err)
	if p == nil || p.Slug != "test_bot" { t.Errorf("got slug=%v", p) }
}
...
func TestPickWelcomeMessage_NilProfile(t *testing.T) {
	...
	if got == "" { t.Error("expected default message") }
}
...
func TestSyncAgentProfile_Nil(t *testing.T) {
	if err := SyncAgentProfile(nil, nil); err != nil { t.Errorf("got %v", err) }
}