...
func newHotSearchService(t *testing.T) (*HotSearchService, *SearchHotRecorder, *gorm.DB) {
	t.Helper()
	db := servicetest.NewDB(t)
	mr, rdb := servicetest.NewRedis(t)
	_ = mr
	rec := &SearchHotRecorder{Rdb: rdb}
	return NewHotSearchService(db, rec), rec, db
}

func TestHotSearchService_OpCRUD(t *testing.T) {
	s, _, _ := newHotSearchService(t)
...
func TestHotSearchService_ReorderItems(t *testing.T) {
	...
	require.NoError(t, db.First(&layout, 1).Error)
...
func TestHotSearchService_RedisPassthrough(t *testing.T) {
	...
	s2 := NewHotSearchService(servicetest.NewDB(t), nil)
