...
func newCreatorCommentService(t *testing.T) (*CreatorCommentService, *gorm.DB) {
	t.Helper()
	db := servicetest.NewDB(t)
	_, rdb := servicetest.NewRedis(t)
	return NewCreatorCommentService(db, rdb, servicetest.ZapNop()), db
}

func TestCreatorComment_VideoComments(t *testing.T) {
	s, db := newCreatorCommentService(t)
	ctx := context.Background()
	servicetest.SeedUser(t, db, 1, "creator")
	servicetest.SeedUser(t, db, 2, "viewer")
	require.NoError(t, db.Create(&video.Video{ID: 10, UserID: 1, Title: "my video", Status: video.StatusPublished}).Error)
	require.NoError(t, db.Create(&comment.Comment{VideoID: 10, UserID: 2, Content: "approved", Approved: true}).Error)
	require.NoError(t, db.Create(&comment.Comment{VideoID: 10, UserID: 2, Content: "pending", Approved: false}).Error)
	...
	// Pending ignored.
	require.NoError(t, db.Model(&comment.Comment{}).Where("content = ?", "pending").
	...
	// Viewer likes.
	var cid uint64
	require.NoError(t, db.Model(&comment.Comment{}).Where("content = ?", "approved").Pluck("id", &cid).Error)
	require.NoError(t, db.Create(&comment.CommentLike{UserID: 2, CommentID: cid}).Error)
...
func TestCreatorComment_ArticleAndDynamic(t *testing.T) {
	...
	require.NoError(t, db.Create(&comment.ArticleComment{ArticleID: 20, UserID: 2, Content: "ac", Approved: true}).Error)
	require.NoError(t, db.Create(&comment.DynamicComment{DynamicID: 30, UserID: 2, Content: "dc", Approved: true}).Error)
...
func TestCreatorComment_BatchFetchesAndCounts(t *testing.T) {
	s, db := newCreatorCommentService(t)
	ctx := context.Background()
	servicetest.SeedUser(t, db, 1, "u1")
	servicetest.SeedUser(t, db, 2, "u2")
	require.NoError(t, db.Create(&video.Video{ID: 10, UserID: 1, Title: "v", Status: video.StatusPublished}).Error)
	require.NoError(t, db.Create(&article.Article{ID: 20, UserID: 1, Title: "a", Status: article.StatusPublished}).Error)
	require.NoError(t, db.Create(&dynamic.UserDynamic{ID: 30, UserID: 1, Title: "d"}).Error)
	require.NoError(t, db.Create(&comment.Comment{VideoID: 10, UserID: 2, Content: "c1", Approved: true}).Error)
	cm := comment.Comment{VideoID: 10, UserID: 2, Content: "c2", Approved: true}
	require.NoError(t, db.Create(&cm).Error)
	require.NoError(t, db.Create(&comment.Comment{VideoID: 10, UserID: 2, Content: "reply", ParentID: cm.ID, Approved: true}).Error)
	require.NoError(t, db.Create(&comment.ArticleComment{ArticleID: 20, UserID: 2, Content: "ac", Approved: true}).Error)
	ac := comment.ArticleComment{ArticleID: 20, UserID: 2, Content: "ac2", Approved: true}
	require.NoError(t, db.Create(&ac).Error)
	require.NoError(t, db.Create(&comment.ArticleComment{ArticleID: 20, UserID: 2, Content: "areply", ParentID: ac.ID, Approved: true}).Error)
	require.NoError(t, db.Create(&comment.DynamicComment{DynamicID: 30, UserID: 2, Content: "dc", Approved: true}).Error)
	dc := comment.DynamicComment{DynamicID: 30, UserID: 2, Content: "dc2", Approved: true}
	require.NoError(t, db.Create(&dc).Error)
	require.NoError(t, db.Create(&comment.DynamicComment{DynamicID: 30, UserID: 2, Content: "dreply", ParentID: dc.ID, Approved: true}).Error)
	...
	require.NoError(t, db.Create(&comment.ArticleCommentLike{UserID: 2, CommentID: ac.ID}).Error)
	alikes, err := s.BatchFetchUserLikesForArticleComments(ctx, 2, []uint64{ac.ID})
	require.NoError(t, err)
	require.Equal(t, map[uint64]bool{ac.ID: true}, alikes)
	require.NoError(t, db.Create(&comment.DynamicCommentLike{UserID: 2, CommentID: dc.ID}).Error)
