...
func TestListHotSearchMerged(t *testing.T) {
	...
	items, err := ListHotSearchMerged(ctx, s.db, rec, 10)
	require.NoError(t, err)
	require.NotEmpty(t, items)
	require.Equal(t, "Go", items[0].Title)

	// Detail variant includes source annotations.
	details, err := ListHotSearchMergedDetail(ctx, s.db, rec, 10)
	require.NoError(t, err)
	require.NotEmpty(t, details)

	// Legacy path directly.
	legacy, err := listHotSearchMergedLegacy(ctx, s.db, rec, 0)
	require.NoError(t, err)
	require.NotEmpty(t, legacy)

	// Flag map.
	flags := ActiveHotSearchOpFlags(s.db)
