...
func newDynamicService(t *testing.T) (*DynamicService, *gorm.DB) {
	t.Helper()
	db := servicetest.NewDB(t)
	_, rdb := servicetest.NewRedis(t)
	return NewDynamicService(db, rdb, servicetest.ZapNop()), db
}

func TestDynamicService_CRUD(t *testing.T) {
	s, db := newDynamicService(t)
	...
	require.NoError(t, db.Create(&comment.UserDynamicLike{UserID: 2, DynamicID: d.ID}).Error)
	cm := comment.DynamicComment{DynamicID: d.ID, UserID: 2, Content: "c1"}
	require.NoError(t, db.Create(&cm).Error)
	require.NoError(t, db.Create(&comment.DynamicCommentLike{UserID: 3, CommentID: cm.ID}).Error)
	require.NoError(t, db.Create(&comment.DynamicCommentDislike{UserID: 4, CommentID: cm.ID}).Error)

	require.NoError(t, s.DeleteDynamic(ctx, d.ID))
	var n int64
	require.NoError(t, db.Model(&dynamic.UserDynamic{}).Count(&n).Error)
	require.Zero(t, n)
	require.NoError(t, db.Model(&comment.UserDynamicLike{}).Count(&n).Error)
	require.Zero(t, n)
	require.NoError(t, db.Model(&comment.DynamicComment{}).Count(&n).Error)
	require.Zero(t, n)
	require.NoError(t, db.Model(&comment.DynamicCommentLike{}).Count(&n).Error)
	require.Zero(t, n)
}

func TestDynamicService_Lists(t *testing.T) {
	s, _ := newDynamicService(t)
	ctx := context.Background()
	for i := 0; i < 3; i++ {
