...
func NewFollowService(db *gorm.DB, log *zap.Logger) *FollowService {
	return &FollowService{store: NewFollowStore(db), log: log}
}

// FollowStore is the follow-domain storage boundary (Phase 1: *gorm.DB impl).
type FollowStore interface {
	GetFollowCounts(ctx context.Context, userID uint64) (FollowCounts, error)
	...
	ToggleFollow(ctx context.Context, followerID, followeeID uint64) (bool, error)
	LoadUser(ctx context.Context, userID uint64) (*user.User, error)
	UsersBlocked(ctx context.Context, a, b uint64) (bool, error)
	...
	GetGroup(ctx context.Context, userID, groupID uint64) (*user.UserFollowGroup, error)
	...
	UpdateGroup(ctx context.Context, userID, groupID uint64, name string) (*user.UserFollowGroup, error)
	DeleteGroup(ctx context.Context, userID, groupID uint64) error
	GetFolloweeGroupIDs(ctx context.Context, userID, followeeID uint64) ([]uint64, error)
	AddGroupMember(ctx context.Context, groupID, followeeID uint64) error
	RemoveGroupMember(ctx context.Context, groupID, followeeID uint64) error
	RemoveFolloweeFromAllGroups(ctx context.Context, ownerID, followeeID uint64) error
	GetFolloweeIDsInGroup(ctx context.Context, groupID uint64) ([]uint64, error)
	BlockUser(ctx context.Context, uid, targetID uint64) error
	UnblockUser(ctx context.Context, uid, targetID uint64) error
}

// FollowStoreImpl implements FollowStore using *gorm.DB (Phase 1 monolith).
...
func (p *FollowStoreImpl) GetFollowCounts(ctx context.Context, userID uint64) (FollowCounts, error) {
	var following, followers int64
	if err := p.db.WithContext(ctx).Model(&user.UserFollow{}).Where("follower_id = ?", userID).Count(&following).Error; err != nil {
		return FollowCounts{}, err
	}
	if err := p.db.WithContext(ctx).Model(&user.UserFollow{}).Where("followee_id = ?", userID).Count(&followers).Error; err != nil {
		return FollowCounts{}, err
	}
...
func (p *FollowStoreImpl) IsFollowing(ctx context.Context, followerID, followeeID uint64) (bool, error) {
	if followerID == 0 || followeeID == 0 || followerID == followeeID {
		return false, nil
	}
	var n int64
	if err := p.db.WithContext(ctx).Model(&user.UserFollow{}).
...
func (p *FollowStoreImpl) GetFollowingIDs(ctx context.Context, followerID uint64, candidateIDs []uint64) (map[uint64]bool, error) {
...
func (p *FollowStoreImpl) GetFollowingList(ctx context.Context, ownerID uint64, limit int, groupID uint64) ([]user.UserFollow, error) {
	q := p.db.WithContext(ctx).Where("follower_id = ?", ownerID)
	if groupID > 0 {
		var memberIDs []uint64
		if err := p.db.WithContext(ctx).Model(&user.UserFollowGroupMember{}).
...
func (p *FollowStoreImpl) GetFollowersList(ctx context.Context, ownerID uint64, limit int) ([]user.UserFollow, error) {
	var rows []user.UserFollow
	if err := p.db.WithContext(ctx).Where("followee_id = ?", ownerID).
...
func (p *FollowStoreImpl) ToggleFollow(ctx context.Context, followerID, followeeID uint64) (bool, error) {
	var row user.UserFollow
	err := p.db.WithContext(ctx).Where("follower_id = ? AND followee_id = ?", followerID, followeeID).First(&row).Error
	if err == nil {
		// Unfollow
...
func (p *FollowStoreImpl) LoadUser(ctx context.Context, userID uint64) (*user.User, error) {
	var u user.User
...
func (p *FollowStoreImpl) UsersBlocked(ctx context.Context, a, b uint64) (bool, error) {
	if a == 0 || b == 0 || a == b {
		return false, nil
	}
	var n int64
	if err := p.db.WithContext(ctx).Model(&user.UserBlock{}).
...
func (p *FollowStoreImpl) GetUploaderPublishedCount(ctx context.Context, userID uint64) (int64, error) {
	var videoN, articleN, dynN int64
	if err := p.db.WithContext(ctx).Model(&video.Video{}).
		Where("user_id = ? AND status = ?", userID, video.StatusPublished).
		Count(&videoN).Error; err != nil {
		return 0, err
	}
	if err := p.db.WithContext(ctx).Model(&article.Article{}).
		Where("user_id = ? AND status = ?", userID, article.StatusPublished).
		Count(&articleN).Error; err != nil {
		return 0, err
	}
	if err := p.db.WithContext(ctx).Model(&dynamic.UserDynamic{}).
...
func (p *FollowStoreImpl) ListGroups(ctx context.Context, userID uint64) ([]user.UserFollowGroup, error) {
	var groups []user.UserFollowGroup
	if err := p.db.WithContext(ctx).Where("user_id = ?", userID).Order("created_at ASC, id ASC").Find(&groups).Error; err != nil {
		return nil, err
	}
	return groups, nil
}
...
func (p *FollowStoreImpl) GetGroup(ctx context.Context, userID, groupID uint64) (*user.UserFollowGroup, error) {
	if groupID == 0 {
		return nil, gorm.ErrRecordNotFound
	}
	var g user.UserFollowGroup
	if err := p.db.WithContext(ctx).Where("id = ? AND user_id = ?", groupID, userID).First(&g).Error; err != nil {
		return nil, err
	}
	return &g, nil
}
...
func (p *FollowStoreImpl) GetGroupMemberCounts(ctx context.Context, groupIDs []uint64) (map[uint64]int64, error) {
...
func (p *FollowStoreImpl) CreateGroup(ctx context.Context, userID uint64, name string) (*user.UserFollowGroup, error) {
	var total int64
	if err := p.db.WithContext(ctx).Model(&user.UserFollowGroup{}).Where("user_id = ?", userID).Count(&total).Error; err != nil {
		return nil, err
	}
	if total >= 50 {
	...
	var dup int64
	if err := p.db.WithContext(ctx).Model(&user.UserFollowGroup{}).
	...
	if err := p.db.WithContext(ctx).Create(&g).Error; err != nil {
		return nil, err
	}
	return &g, nil
}
...
func (p *FollowStoreImpl) UpdateGroup(ctx context.Context, userID, groupID uint64, name string) (*user.UserFollowGroup, error) {
	g, err := p.GetGroup(ctx, userID, groupID)
	if err != nil {
		return nil, err
	}
	if err := p.db.WithContext(ctx).Model(g).Update("name", name).Error; err != nil {
		return nil, err
	}
	return p.GetGroup(ctx, userID, groupID)
}

// DeleteGroup deletes a follow group and its members, verifying ownership.
func (p *FollowStoreImpl) DeleteGroup(ctx context.Context, userID, groupID uint64) error {
	g, err := p.GetGroup(ctx, userID, groupID)
	if err != nil {
		return err
	}
...
func (p *FollowStoreImpl) GetFolloweeGroupIDs(ctx context.Context, userID, followeeID uint64) ([]uint64, error) {
	var groupIDs []uint64
	err := p.db.WithContext(ctx).Model(&user.UserFollowGroupMember{}).
...
func (p *FollowStoreImpl) AddGroupMember(ctx context.Context, groupID, followeeID uint64) error {
	var g user.UserFollowGroup
	err := p.db.WithContext(ctx).First(&g, groupID).Error // ownership check done by caller
	if err != nil {
		return err
	}
	var existing user.UserFollowGroupMember
	err = p.db.WithContext(ctx).Where("group_id = ? AND followee_id = ?", groupID, followeeID).First(&existing).Error
	if err == nil {
	...
	return p.db.WithContext(ctx).Create(&user.UserFollowGroupMember{GroupID: groupID, FolloweeID: followeeID}).Error
}

// RemoveGroupMember removes a followee from a follow group.
func (p *FollowStoreImpl) RemoveGroupMember(ctx context.Context, groupID, followeeID uint64) error {
	return p.db.WithContext(ctx).Where("group_id = ? AND followee_id = ?", groupID, followeeID).
		Delete(&user.UserFollowGroupMember{}).Error
}

// RemoveFolloweeFromAllGroups removes a followee from all groups owned by the user.
func (p *FollowStoreImpl) RemoveFolloweeFromAllGroups(ctx context.Context, ownerID, followeeID uint64) error {
	var groupIDs []uint64
	if err := p.db.WithContext(ctx).Model(&user.UserFollowGroup{}).
	...
	return p.db.WithContext(ctx).Where("group_id IN ? AND followee_id = ?", groupIDs, followeeID).
		Delete(&user.UserFollowGroupMember{}).Error
}

// GetFolloweeIDsInGroup returns followee IDs in a group.
func (p *FollowStoreImpl) GetFolloweeIDsInGroup(ctx context.Context, groupID uint64) ([]uint64, error) {
	var ids []uint64
	err := p.db.WithContext(ctx).Model(&user.UserFollowGroupMember{}).
...
func (p *FollowStoreImpl) BlockUser(ctx context.Context, uid, targetID uint64) error {
	return p.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
...
func (p *FollowStoreImpl) UnblockUser(ctx context.Context, uid, targetID uint64) error {
	return p.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
