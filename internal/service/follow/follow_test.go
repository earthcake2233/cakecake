...
func newFollowService(t *testing.T) (*FollowService, *gorm.DB) {
	t.Helper()
	db := servicetest.NewDB(t)
	return NewFollowService(db, servicetest.ZapNop()), db
}
...
func TestFollowService_GetFollowingList(t *testing.T) {
	...
	require.NoError(t, db.Create(&user.UserFollowGroupMember{GroupID: g.ID, FolloweeID: 2}).Error)
...
func TestFollowService_ToggleFollow(t *testing.T) {
	...
	require.False(t, ok)
	var n int64
	require.NoError(t, db.Model(&user.UserFollow{}).Count(&n).Error)
	require.Zero(t, n)
	...
	require.NoError(t, err)
	var members int64
	require.NoError(t, db.Model(&user.UserFollowGroupMember{}).Count(&members).Error)
...
func TestFollowService_LoadUser(t *testing.T) {
	...
	now := time.Now()
	require.NoError(t, db.Model(&user.User{}).Where("id = ?", 1).
...
func TestFollowService_UsersBlocked(t *testing.T) {
	...
	require.NoError(t, db.Create(&user.UserBlock{BlockerID: 1, BlockedID: 2}).Error)
...
func TestFollowService_GetUploaderPublishedCount(t *testing.T) {
	...
	require.NoError(t, db.Create(&video.Video{UserID: 1, Title: "v1", Status: video.StatusPublished}).Error)
	require.NoError(t, db.Create(&video.Video{UserID: 1, Title: "v2", Status: video.StatusDraft}).Error)
	require.NoError(t, db.Create(&article.Article{UserID: 1, Title: "a1", Status: article.StatusPublished}).Error)
	require.NoError(t, db.Create(&dynamic.UserDynamic{UserID: 1, Title: "d1"}).Error)
...
func TestFollowService_GroupsCRUD(t *testing.T) {
	...
	// Member counts.
	require.NoError(t, db.Create(&user.UserFollowGroupMember{GroupID: g1.ID, FolloweeID: 2}).Error)
	require.NoError(t, db.Create(&user.UserFollowGroupMember{GroupID: g1.ID, FolloweeID: 3}).Error)
