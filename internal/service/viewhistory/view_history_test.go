...
func newViewHistoryService(t *testing.T) (*ViewHistoryService, *gorm.DB) {
	t.Helper()
	db := servicetest.NewDB(t)
	_, rdb := servicetest.NewRedis(t)
	return NewViewHistoryService(db, rdb, servicetest.ZapNop()), db
}

func TestViewHistory_RecordAndList(t *testing.T) {
	s, db := newViewHistoryService(t)
	...
	var n int64
	require.NoError(t, db.Model(&extra.VideoViewHistory{}).Where("user_id = ?", 1).Count(&n).Error)
...
func TestViewHistory_RecordWithProgress(t *testing.T) {
	...
	require.NoError(t, db.Where("user_id = ? AND video_id = ?", 1, 10).First(&row).Error)
...
func TestViewHistory_RecordArticle(t *testing.T) {
	...
	var n int64
	require.NoError(t, db.Model(&extra.ArticleViewHistory{}).Count(&n).Error)
	require.Zero(t, n)
	...
	require.NoError(t, db.Where("user_id = ? AND article_id = ?", 1, 5).First(&row).Error)
	require.Equal(t, "web", row.Device)

	// Paused user -> no record.
	require.NoError(t, db.Model(&user.User{}).Where("id = ?", 1).Update("view_history_paused", true).Error)
	s.RecordArticleViewHistory(ctx, 1, 6, "web")
	require.NoError(t, db.Model(&extra.ArticleViewHistory{}).Where("article_id = ?", 6).Count(&n).Error)
	require.Zero(t, n)
}
...
func TestViewHistory_TrimCombined(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		require.NoError(t, db.Create(&extra.VideoViewHistory{
			UserID: 1, VideoID: uint64(10 + i), ViewedAt: base.Add(time.Duration(i) * time.Minute),
		}).Error)
		require.NoError(t, db.Create(&extra.ArticleViewHistory{
			UserID: 1, ArticleID: uint64(20 + i), ViewedAt: base.Add(time.Duration(i) * time.Minute),
		}).Error)
	}
	s.TrimViewHistoryCombined(ctx, 1, 4)
	var vn, an int64
	require.NoError(t, db.Model(&extra.VideoViewHistory{}).Where("user_id = ?", 1).Count(&vn).Error)
	require.NoError(t, db.Model(&extra.ArticleViewHistory{}).Where("user_id = ?", 1).Count(&an).Error)
...
func TestViewHistory_KeywordSearch(t *testing.T) {
	...
	require.NoError(t, db.Create(&video.Video{ID: 10, Title: "golang tutorial", Status: video.StatusPublished}).Error)
	require.NoError(t, db.Create(&article.Article{ID: 20, Title: "golang notes", Status: article.StatusPublished}).Error)
	require.NoError(t, db.Create(&extra.VideoViewHistory{UserID: 1, VideoID: 10, ViewedAt: time.Now()}).Error)
	require.NoError(t, db.Create(&extra.ArticleViewHistory{UserID: 1, ArticleID: 20, ViewedAt: time.Now()}).Error)
...
func TestViewHistory_BatchFetches(t *testing.T) {
	...
	require.NoError(t, db.Create(&video.Video{ID: 10, Title: "v", Status: video.StatusPublished}).Error)
	require.NoError(t, db.Create(&article.Article{ID: 20, Title: "a", Status: article.StatusPublished}).Error)
	servicetest.SeedUser(t, db, 30, "alice")
...
func TestViewHistory_DeleteAndClear(t *testing.T) {
	...
	var n int64
	require.NoError(t, db.Model(&extra.VideoViewHistory{}).Count(&n).Error)
	require.Zero(t, n)
	require.NoError(t, db.Model(&extra.ArticleViewHistory{}).Count(&n).Error)
	require.Zero(t, n)
}
