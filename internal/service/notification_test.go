...
func newNotificationService(t *testing.T) *NotificationService {
	t.Helper()
	db := newAgentTestDB(t)
	_, rdb := newAgentTestRedis(t)
	return NewNotificationService(db, rdb, zapNop(), NewUserProvider(db))
}

func TestNotification_NotifyVideoComment(t *testing.T) {
	ns := newNotificationService(t)
	...
	var n int64
	require.NoError(t, ns.db.Model(&notification.Notification{}).Count(&n).Error)
	require.Zero(t, n)
	...
	require.NoError(t, ns.db.First(&row).Error)
...
func TestNotification_NotifyCommentReply(t *testing.T) {
	...
	require.NoError(t, ns.db.Create(&parent).Error)
...
func TestNotification_NotifyCommentLike(t *testing.T) {
	ns := newNotificationService(t)
	ctx := context.Background()
	seedUser(t, ns.db, 1, "alice")
	seedUser(t, ns.db, 2, "bob")
	seedUser(t, ns.db, 3, "carol")
	seedUser(t, ns.db, 4, "dave")
	...
	require.NoError(t, ns.db.Create(&notification.LikeNotifMute{RecipientID: 1, CommentID: 5}).Error)
...
func TestNotification_ListAndRead(t *testing.T) {
	ns := newNotificationService(t)
	ctx := context.Background()
	require.NoError(t, ns.db.Create(&notification.Notification{RecipientID: 1, Type: "reply"}).Error)
	require.NoError(t, ns.db.Create(&notification.Notification{RecipientID: 1, Type: "reply_like"}).Error)
	require.NoError(t, ns.db.Create(&notification.Notification{RecipientID: 1, Type: "at"}).Error)
	require.NoError(t, ns.db.Create(&notification.Notification{RecipientID: 1, Type: "like_aggregation"}).Error)
	require.NoError(t, ns.db.Create(&notification.Notification{RecipientID: 1, Type: "system"}).Error)
	require.NoError(t, ns.db.Create(&notification.Notification{RecipientID: 1, Type: "dm"}).Error)
...
func TestNotification_MuteAndLikers(t *testing.T) {
	...
	seedUser(t, ns.db, 1, "alice")
	seedUser(t, ns.db, 2, "bob")
	require.NoError(t, ns.db.Create(&comment.CommentLike{UserID: 2, CommentID: 5}).Error)
	require.NoError(t, ns.db.Create(&notification.Notification{
		RecipientID: 1, Type: "like_aggregation", RelatedID: 5, PayloadJSON: "like_comment:5",
	}).Error)

	// Not a like aggregation -> param error.
	require.NoError(t, ns.db.Create(&notification.Notification{RecipientID: 1, Type: "reply", RelatedID: 6}).Error)
	var replyID uint64
	require.NoError(t, ns.db.Model(&notification.Notification{}).Where("type = ?", "reply").Pluck("id", &replyID).Error)
	err := ns.MuteLikeNotification(ctx, 1, replyID)
	require.ErrorIs(t, err, ErrParamError)

	// Mute aggregation -> creates mute row.
	var aggID uint64
	require.NoError(t, ns.db.Model(&notification.Notification{}).Where("type = ?", "like_aggregation").Pluck("id", &aggID).Error)
