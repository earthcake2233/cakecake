...
func newAgentServiceForTest(t *testing.T) *AgentService {
	t.Helper()
	return &AgentService{DB: newAgentTestDB(t), Log: zapNop()}
}

func TestAgentService_SimpleHelpers(t *testing.T) {
	s := newAgentServiceForTest(t)
...
func TestAgentService_BotUserAndSync(t *testing.T) {
	...
	var u user.User
	require.NoError(t, s.DB.First(&u, botID).Error)
...
func TestAgentService_GlobalPrompt(t *testing.T) {
	...
	require.NoError(t, data.EnsureDefaultAgentSettings(s.DB, zapNop()))
	require.NoError(t, s.DB.Model(&agent.AgentSettings{}).Where("id = ?", 1).
