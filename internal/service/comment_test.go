...
func newCommentService(t *testing.T) *CommentService {
	t.Helper()
	db := newAgentTestDB(t)
	...
	notif := NewNotificationService(db, rdb, zapNop(), NewUserProvider(db))
	return NewCommentService(db, rdb, zapNop(), filter, notif,
		NewUserProvider(db), NewVideoProvider(db), NewArticleProvider(db), NewDynamicProvider(db))
}

func seedCommentTargets(t *testing.T, s *CommentService) {
	t.Helper()
	seedUser(t, s.db, 1, "owner")
...
func TestCommentService_PostAndList(t *testing.T) {
	s := newCommentService(t)
	ctx := context.Background()
	seedCommentTargets(t, s)
	...
	require.ErrorIs(t, err, ErrCommentSensitive)
	// Missing video.
	...
	// Comments closed.
	require.NoError(t, s.db.Model(&video.Video{}).Where("id = ?", 10).Update("comments_closed", true).Error)
	_, err = s.PostComment(ctx, 2, 10, PostCommentReq{Content: "x"}, "")
	require.ErrorIs(t, err, ErrCommentsClosed)
	res, err := s.ListComments(ctx, 10, 2)
	require.NoError(t, err)
	require.True(t, res.CommentsClosed)
	require.Empty(t, res.Items)
	require.NoError(t, s.db.Model(&video.Video{}).Where("id = ?", 10).Update("comments_closed", false).Error)
...
func TestCommentService_DeletePin(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, s.db.Create(&comment.CommentLike{UserID: 3, CommentID: cm.ID}).Error)
	require.NoError(t, s.db.Create(&comment.CommentLike{UserID: 3, CommentID: reply.ID}).Error)

	// Non-owner, non-uploader -> forbidden.
	seedUser(t, s.db, 5, "intruder")
	...
	require.NoError(t, err)
	var n int64
	require.NoError(t, s.db.Model(&comment.Comment{}).Count(&n).Error)
	require.Zero(t, n)
	require.NoError(t, s.db.Model(&comment.CommentLike{}).Count(&n).Error)
	require.Zero(t, n)
...
func TestCommentService_Reactions(t *testing.T) {
	...
	require.NoError(t, s.db.First(&row, cm.ID).Error)
