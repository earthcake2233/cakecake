...
func newUserService(t *testing.T) (*UserService, *gorm.DB) {
	...
	return NewUserService(db, servicetest.ZapNop()), db
}

func TestUserService_GetMeAndUpdate(t *testing.T) {
	s, db := newUserService(t)
...
func TestUserService_CoinLedger(t *testing.T) {
	...
	ctx := context.Background()
	servicetest.SeedUser(t, db, 1, "alice")
	require.NoError(t, db.Create(&user.CoinLedger{UserID: 1, DeltaTenths: -10}).Error)
	require.NoError(t, db.Create(&user.CoinLedger{UserID: 1, DeltaTenths: 30}).Error)
