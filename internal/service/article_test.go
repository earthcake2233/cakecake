...
func newArticleService(t *testing.T) *ArticleService {
	t.Helper()
	db := newAgentTestDB(t)
	_, rdb := newAgentTestRedis(t)
	return NewArticleService(db, rdb, zapNop(), NewUserService(db, zapNop()), nil)
}
...
func TestArticleService_AdminAndDelete(t *testing.T) {
	...
	require.NoError(t, s.db.Create(&comment.ArticleComment{ArticleID: 10, UserID: 2, Content: "c", Approved: true}).Error)
	...
	var cn int64
	require.NoError(t, s.db.Model(&comment.ArticleComment{}).Where("article_id = ?", 10).Count(&cn).Error)
	require.Zero(t, cn)
