...
func newDanmakuService(t *testing.T) (*DanmakuService, *sensitive.Filter) {
	t.Helper()
	db := newAgentTestDB(t)
	...
	return NewDanmakuService(db, rdb, zapNop(), filter), filter
}

func TestDanmakuService_PostDanmaku(t *testing.T) {
	s, filter := newDanmakuService(t)
	ctx := context.Background()

	v := video.Video{ID: 10, UserID: 1, Title: "v", Status: video.StatusPublished}
	require.NoError(t, s.db.Create(&v).Error)
	seedUser(t, s.db, 1, "alice")

	// Missing video.
	_, err := s.PostDanmaku(ctx, 999, 1, "hi", "", "scroll", "", 1.0)
	require.Error(t, err)
	require.Equal(t, 40400, err.(*SvcError).Code)

	// Danmaku closed.
	require.NoError(t, s.db.Model(&video.Video{}).Where("id = ?", 10).Update("danmaku_closed", true).Error)
	_, err = s.PostDanmaku(ctx, 10, 1, "hi", "", "scroll", "", 1.0)
	require.Equal(t, 40304, err.(*SvcError).Code)
	require.NoError(t, s.db.Model(&video.Video{}).Where("id = ?", 10).Update("danmaku_closed", false).Error)
	...
	require.Equal(t, 40022, err.(*SvcError).Code)
	...
	require.Equal(t, 40025, err.(*SvcError).Code)
...
func TestDanmakuService_ListCreatorDanmakus(t *testing.T) {
	...
	require.NoError(t, s.db.Create(&video.Video{ID: 10, UserID: 1, Title: "v1", CoverURL: "c", Status: video.StatusPublished}).Error)
	require.NoError(t, s.db.Create(&video.Video{ID: 11, UserID: 2, Title: "v2", Status: video.StatusPublished}).Error)
	require.NoError(t, s.db.Create(&danmaku.Danmaku{VideoID: 10, UserID: 2, Content: "hello world", Type: "scroll"}).Error)
	require.NoError(t, s.db.Create(&danmaku.Danmaku{VideoID: 10, UserID: 2, Content: "top stuff", Type: "top"}).Error)
	require.NoError(t, s.db.Create(&danmaku.Danmaku{VideoID: 11, UserID: 1, Content: "other", Type: "scroll"}).Error)
...
func TestDanmakuService_DeleteCreatorDanmaku(t *testing.T) {
	...
	require.NoError(t, s.db.Create(&d).Error)
	require.NoError(t, s.db.Create(&danmaku.DanmakuLike{UserID: 3, DanmakuID: d.ID}).Error)
	...
	var n int64
	require.NoError(t, s.db.Model(&danmaku.DanmakuLike{}).Count(&n).Error)
	require.Zero(t, n)
...
func TestDanmakuService_ListHistory(t *testing.T) {
	s, _ := newDanmakuService(t)
	ctx := context.Background()
	require.NoError(t, s.db.Create(&danmaku.Danmaku{VideoID: 10, UserID: 1, Content: "a", VideoTime: 1.0}).Error)
	require.NoError(t, s.db.Create(&danmaku.Danmaku{VideoID: 10, UserID: 1, Content: "b", VideoTime: 5.0}).Error)
	require.NoError(t, s.db.Create(&danmaku.Danmaku{VideoID: 10, UserID: 1, Content: "c", VideoTime: 20.0}).Error)
