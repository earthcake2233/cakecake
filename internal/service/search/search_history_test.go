...
func newSearchHistoryService(t *testing.T) (*SearchHistoryService, *gorm.DB) {
	t.Helper()
	db := servicetest.NewDB(t)
	return NewSearchHistoryService(db, servicetest.ZapNop()), db
}

func TestSearchHistory_UpsertAndList(t *testing.T) {
	s, db := newSearchHistoryService(t)
	...
	require.NoError(t, db.Create(&extra.UserSearchHistory{
		UserID: 1, Keyword: "Rust", KeywordNorm: "rust", UpdatedAt: at,
	}).Error)
	require.NoError(t, s.UpsertKeyword(ctx, 1, "rust", at.Add(2*time.Minute)))
	var n int64
	require.NoError(t, db.Model(&extra.UserSearchHistory{}).Where("keyword_norm = ?", "rust").Count(&n).Error)
...
func TestSearchHistory_Trim(t *testing.T) {
	...
	for i := 0; i < maxUserSearchHistory+5; i++ {
		require.NoError(t, s.UpsertKeyword(ctx, 1, string(rune('a'+i%26))+strconv.FormatUint(uint64(i), 10), at.Add(time.Duration(i)*time.Second)))
	}
	require.NoError(t, s.TrimHistory(ctx, 1))
	var n int64
	require.NoError(t, db.Model(&extra.UserSearchHistory{}).Where("user_id = ?", 1).Count(&n).Error)
	require.Equal(t, int64(maxUserSearchHistory), n)
}

func TestSearchHistory_Replace(t *testing.T) {
	s, _ := newSearchHistoryService(t)
	ctx := context.Background()
