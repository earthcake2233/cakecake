...
func TestGetUserPublic(t *testing.T) {
	...
	owner := user.User{Username: "spaceowner", PasswordHash: "hash", Nickname: "SpaceOwner",
		Sign: "hello world", Gender: "male", Birthday: "2000-01-01", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)
	api.DB.Model(&owner).Update("cake_id", user.FormatCakeID(owner.ID))

	vid := video.Video{UserID: owner.ID, Title: "Space Video", Status: "published",
...
func TestListUserPublishedVideos(t *testing.T) {
	...
	owner := user.User{Username: "vidowner", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	for i := 0; i < 3; i++ {
		v := video.Video{UserID: owner.ID, Title: fmt.Sprintf("Pub %d", i), Status: "published",
			VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
		require.NoError(t, api.DB.Create(&v).Error)
	}
	draft := video.Video{UserID: owner.ID, Title: "Draft", Status: "draft",
...
func TestListUserFavoriteFolders(t *testing.T) {
	...
	owner := user.User{Username: "favfolderuser", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	folder := video.FavoriteFolder{UserID: owner.ID, Title: "My Favs", IsPublic: true}
...
func TestListUserFavorites(t *testing.T) {
	...
	owner := user.User{Username: "favuser", PasswordHash: "hash", PrivacyPublicFavorites: true, CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	uploader := user.User{Username: "uploader1", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&uploader).Error)

	v := video.Video{UserID: uploader.ID, Title: "Fav Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	folder := video.FavoriteFolder{UserID: owner.ID, Title: "Default", IsPublic: true, IsDefault: true}
	require.NoError(t, api.DB.Create(&folder).Error)

	fav := video.VideoFavorite{UserID: owner.ID, VideoID: v.ID, FolderID: folder.ID}
...
func TestListUserRecentCoinVideos(t *testing.T) {
	...
	owner := user.User{Username: "coinuser1", PasswordHash: "hash", PrivacyPublicRecentCoins: true, CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)
	access, _, _, err := jm.IssuePair(owner.ID)
	require.NoError(t, err)

	uploader := user.User{Username: "coinup1", PasswordHash: "hash", CoinBalanceTenths: 2300}
	require.NoError(t, api.DB.Create(&uploader).Error)

	v := video.Video{UserID: uploader.ID, Title: "Coined Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	coin := video.VideoCoin{UserID: owner.ID, VideoID: v.ID, Amount: 1}
...
func TestListUserFollowing(t *testing.T) {
	...
	owner := user.User{Username: "followowner1", PasswordHash: "hash", PrivacyPublicFollowing: true, CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	target := user.User{Username: "followtarget1", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&target).Error)

	follow := user.UserFollow{FollowerID: owner.ID, FolloweeID: target.ID}
...
func TestListUserFollowers(t *testing.T) {
	...
	owner := user.User{Username: "followowner2", PasswordHash: "hash", PrivacyPublicFans: true, CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	follower := user.User{Username: "follower1", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&follower).Error)

	follow := user.UserFollow{FollowerID: follower.ID, FolloweeID: owner.ID}
	require.NoError(t, api.DB.Create(&follow).Error)
...
func TestGetVideoDetail(t *testing.T) {
	...
	owner := user.User{Username: "videowner1", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	v := video.Video{UserID: owner.ID, Title: "Detail Video", Status: "published",
	...
	viewer := user.User{Username: "viewer1", PasswordHash: "hash", CoinBalanceTenths: 230}
...
func TestToggleVideoLike(t *testing.T) {
	...
	owner := user.User{Username: "vlikeowner", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	v := video.Video{UserID: owner.ID, Title: "Like Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	viewer := user.User{Username: "vlikeviewer", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&viewer).Error)
	access, _, _, err := jm.IssuePair(viewer.ID)
	require.NoError(t, err)
...
func TestPostVideoCoin(t *testing.T) {
	...
	owner := user.User{Username: "vcoinowner", PasswordHash: "hash", CoinBalanceTenths: 2300}
	require.NoError(t, api.DB.Create(&owner).Error)

	v := video.Video{UserID: owner.ID, Title: "Coin Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	viewer := user.User{Username: "vcoinviewer", PasswordHash: "hash", CoinBalanceTenths: 2300}
	require.NoError(t, api.DB.Create(&viewer).Error)
	access, _, _, err := jm.IssuePair(viewer.ID)
	require.NoError(t, err)
...
func TestToggleWatchLater(t *testing.T) {
	...
	owner := user.User{Username: "wlowner", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	v := video.Video{UserID: owner.ID, Title: "WL Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	viewer := user.User{Username: "wlviewer", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&viewer).Error)
	access, _, _, err := jm.IssuePair(viewer.ID)
	require.NoError(t, err)
...
func TestToggleVideoFavorite(t *testing.T) {
	...
	owner := user.User{Username: "vfavowner", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	v := video.Video{UserID: owner.ID, Title: "Fav Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	viewer := user.User{Username: "vfavviewer", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&viewer).Error)
	access, _, _, err := jm.IssuePair(viewer.ID)
	require.NoError(t, err)
...
func TestSetVideoFavoriteFolders(t *testing.T) {
	...
	owner := user.User{Username: "sffowner", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	v := video.Video{UserID: owner.ID, Title: "SFav Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	viewer := user.User{Username: "sffviewer", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&viewer).Error)
	access, _, _, err := jm.IssuePair(viewer.ID)
	require.NoError(t, err)

	folder := video.FavoriteFolder{UserID: viewer.ID, Title: "My Collection", IsPublic: true}
	require.NoError(t, api.DB.Create(&folder).Error)
...
func TestListCommentsMoreVariants(t *testing.T) {
	...
	owner := user.User{Username: "cmtown", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	v := video.Video{UserID: owner.ID, Title: "CM Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now(),
		CommentsCurated: true}
	require.NoError(t, api.DB.Create(&v).Error)

	cmt := comment.Comment{VideoID: v.ID, UserID: owner.ID, Content: "Pending", Approved: false, Level: 1}
	require.NoError(t, api.DB.Create(&cmt).Error)
	cmt2 := comment.Comment{VideoID: v.ID, UserID: owner.ID, Content: "Approved!", Approved: true, Level: 1}
	...
	v2 := video.Video{UserID: owner.ID, Title: "Closed CM", Status: "published",
...
func TestPostCommentExceptions(t *testing.T) {
	...
	owner := user.User{Username: "cmtpostown", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	v := video.Video{UserID: owner.ID, Title: "Post CM", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	user := user.User{Username: "cmtpostuser", PasswordHash: "hash", CoinBalanceTenths: 230}
	...
	v2 := video.Video{UserID: owner.ID, Title: "Closed Post", Status: "published",
...
func TestPinComment(t *testing.T) {
	...
	owner := user.User{Username: "pinowner", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)
	ownerAccess, _, _, err := jm.IssuePair(owner.ID)
	require.NoError(t, err)

	v := video.Video{UserID: owner.ID, Title: "Pin Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	cmt := comment.Comment{VideoID: v.ID, UserID: owner.ID, Content: "Pin me!", Approved: true, Level: 1}
	require.NoError(t, api.DB.Create(&cmt).Error)
	...
	other := user.User{Username: "pinother", PasswordHash: "hash", CoinBalanceTenths: 230}
...
func TestLikeCommentOperations(t *testing.T) {
	...
	owner := user.User{Username: "likecmtown", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	v := video.Video{UserID: owner.ID, Title: "Like CM", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	cmt := comment.Comment{VideoID: v.ID, UserID: owner.ID, Content: "Like target", Approved: true, Level: 1}
	require.NoError(t, api.DB.Create(&cmt).Error)

	user := user.User{Username: "likeuser", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&user).Error)
	access, _, _, err := jm.IssuePair(user.ID)
	require.NoError(t, err)
	...
	var likeCount int64
	api.DB.Model(&comment.CommentLike{}).Where("user_id = ? AND comment_id = ?", user.ID, cmt.ID).Count(&likeCount)
...
func TestGetArticle(t *testing.T) {
	...
	author := user.User{Username: "artauthor", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&author).Error)

	art := article.Article{UserID: author.ID, Title: "Test Article", BodyMD: "# Hello\nWorld",
	...
	viewer := user.User{Username: "artviewer", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&viewer).Error)
	access, _, _, err := jm.IssuePair(viewer.ID)
	require.NoError(t, err)
...
func TestPostArticleView(t *testing.T) {
	...
	author := user.User{Username: "artviewauth", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&author).Error)

	art := article.Article{UserID: author.ID, Title: "View Article", BodyMD: "# V",
		Status: "published", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&art).Error)

	viewer := user.User{Username: "aviewer", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&viewer).Error)
	access, _, _, err := jm.IssuePair(viewer.ID)
	require.NoError(t, err)
...
func TestToggleArticleFavorite(t *testing.T) {
	...
	author := user.User{Username: "artfavauth", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&author).Error)

	art := article.Article{UserID: author.ID, Title: "Fav Article", BodyMD: "# F",
		Status: "published", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&art).Error)

	user := user.User{Username: "favuserart", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&user).Error)
	access, _, _, err := jm.IssuePair(user.ID)
	require.NoError(t, err)
...
func TestPostArticleCoin(t *testing.T) {
	...
	author := user.User{Username: "artcoinauth", PasswordHash: "hash", CoinBalanceTenths: 2300}
	require.NoError(t, api.DB.Create(&author).Error)

	art := article.Article{UserID: author.ID, Title: "Coin Article", BodyMD: "# C",
		Status: "published", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&art).Error)

	user := user.User{Username: "coinuserart", PasswordHash: "hash", CoinBalanceTenths: 2300}
	require.NoError(t, api.DB.Create(&user).Error)
	access, _, _, err := jm.IssuePair(user.ID)
	require.NoError(t, err)
...
func TestBlockUser(t *testing.T) {
	...
	blocker := user.User{Username: "blocker", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&blocker).Error)
	access, _, _, err := jm.IssuePair(blocker.ID)
	require.NoError(t, err)

	target := user.User{Username: "blocktarget", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&target).Error)
...
func TestFollowUser(t *testing.T) {
	...
	follower := user.User{Username: "follower", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&follower).Error)
	access, _, _, err := jm.IssuePair(follower.ID)
	require.NoError(t, err)

	target := user.User{Username: "followtarget", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&target).Error)
...
func TestListCreatorComments(t *testing.T) {
	...
	owner := user.User{Username: "creatorcm", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)
	ownerAccess, _, _, err := jm.IssuePair(owner.ID)
	require.NoError(t, err)

	v := video.Video{UserID: owner.ID, Title: "Creator CM Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CommentsCurated: true,
		CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	cmt := comment.Comment{VideoID: v.ID, UserID: owner.ID, Content: "Creator comment",
...
func TestApproveAndIgnoreCuratedComment(t *testing.T) {
	...
	owner := user.User{Username: "cmapprove", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)
	ownerAccess, _, _, err := jm.IssuePair(owner.ID)
	require.NoError(t, err)

	v := video.Video{UserID: owner.ID, Title: "Approve CM Video", Status: "published",
		VideoURL: "https://cdn.example.com/v.mp4", CommentsCurated: true,
		CreatedAt: time.Now(), UpdatedAt: time.Now()}
	require.NoError(t, api.DB.Create(&v).Error)

	cmt := comment.Comment{VideoID: v.ID, UserID: owner.ID, Content: "Pending approval",
		Approved: false, Level: 1}
	require.NoError(t, api.DB.Create(&cmt).Error)
	...
	cmt2 := comment.Comment{VideoID: v.ID, UserID: owner.ID, Content: "To ignore",
		Approved: false, Level: 1}
	require.NoError(t, api.DB.Create(&cmt2).Error)
...
func TestListUserPublishedArticles(t *testing.T) {
	...
	owner := user.User{Username: "pubartowner", PasswordHash: "hash", CoinBalanceTenths: 230}
	require.NoError(t, api.DB.Create(&owner).Error)

	for i := 0; i < 2; i++ {
		art := article.Article{UserID: owner.ID, Title: fmt.Sprintf("Pub Art %d", i), BodyMD: "# A",
			Status: "published", CreatedAt: time.Now(), UpdatedAt: time.Now()}
		require.NoError(t, api.DB.Create(&art).Error)
	}
	draft := article.Article{UserID: owner.ID, Title: "Draft Art", BodyMD: "# D",
