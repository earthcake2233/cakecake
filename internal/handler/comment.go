...
func errCodeFromSvc(err error) int {
	...
	if errors.As(err, &se) { return se.Code }
	return errcode.CodeInternalError
}

func httpStatusFromSvc(code int) int {
	switch code {
	case errcode.CodeNotFound: return http.StatusNotFound
	case errcode.CodeForbidden: return http.StatusForbidden
	case errcode.CodeUnauthorized: return http.StatusUnauthorized
	case errcode.CodeParamError, errcode.CodeCommentSensitive: return http.StatusBadRequest
	case errcode.CodeCommentsClosed: return http.StatusForbidden
	default: return http.StatusInternalServerError
	}
}

func (a *API) resolveCommentIPLocation(c *gin.Context) string {
	if a.IPLocate == nil { return "" }
...
func (a *API) ApproveComment(c *gin.Context) {
	cid, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil || cid == 0 { resp.Err(c, http.StatusBadRequest, errcode.CodeParamError); return }
	if err := a.CommentSvc.ApproveComment(c.Request.Context(), cid); err != nil { resp.Err(c, http.StatusInternalServerError, errcode.CodeInternalError); return }
	resp.OK(c, gin.H{"ok": true})
}
...
func (a *API) ListNotifications(c *gin.Context) {
	...
	if err != nil { resp.Err(c, http.StatusInternalServerError, errcode.CodeInternalError); return }
...
func (a *API) MarkNotificationsReadBatch(c *gin.Context) {
	...
	if !ok { resp.Err(c, http.StatusUnauthorized, errcode.CodeUnauthorized); return }
	var ids []uint64
	if err := c.ShouldBindJSON(&ids); err != nil { resp.Err(c, http.StatusBadRequest, errcode.CodeParamError); return }
