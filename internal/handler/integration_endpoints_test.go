...
func TestIntegration_BlockUser(t *testing.T) {
	...
	u1 := user.User{Username: "blocker", PasswordHash: "hash", Nickname: "Blocker", CoinBalanceTenths: 100}
	require.NoError(t, api.DB.Create(&u1).Error)
	u2 := user.User{Username: "blocked", PasswordHash: "hash", Nickname: "Blocked", CoinBalanceTenths: 100}
...
func TestIntegration_BlockSelf(t *testing.T) {
	...
	u := user.User{Username: "self", PasswordHash: "hash", Nickname: "Self", CoinBalanceTenths: 100}
...
func TestIntegration_GetMeSpacePrivacy(t *testing.T) {
	api, r, tm := newTestAPI(t)
	u := user.User{Username: "priv", PasswordHash: "hash", Nickname: "Priv", CoinBalanceTenths: 100,
...
func TestIntegration_UpdateMeSpacePrivacy(t *testing.T) {
	api, r, tm := newTestAPI(t)
	u := user.User{Username: "priv2", PasswordHash: "hash", Nickname: "Priv2", CoinBalanceTenths: 100}
	require.NoError(t, api.DB.Create(&u).Error)
	token, _, _, _ := tm.IssuePair(u.ID)
...
func TestIntegration_BlockUser_BlockTwice(t *testing.T) {
	api, r, tm := newTestAPI(t)
	u1 := user.User{Username: "bt1", PasswordHash: "hash", Nickname: "BT1", CoinBalanceTenths: 100}
	u2 := user.User{Username: "bt2", PasswordHash: "hash", Nickname: "BT2", CoinBalanceTenths: 100}
	require.NoError(t, api.DB.Create(&u1).Error)
	require.NoError(t, api.DB.Create(&u2).Error)
...
func TestIntegration_BlockUser_BlockNonexistent(t *testing.T) {
	api, r, tm := newTestAPI(t)
	u := user.User{Username: "bn1", PasswordHash: "hash", Nickname: "BN1", CoinBalanceTenths: 100}
	require.NoError(t, api.DB.Create(&u).Error)
	token, _, _, _ := tm.IssuePair(u.ID)
...
func TestIntegration_SearchHistory(t *testing.T) {
	api, r, tm := newTestAPI(t)
	u := user.User{Username: "sh1", PasswordHash: "hash", Nickname: "SH1", CoinBalanceTenths: 100}
	require.NoError(t, api.DB.Create(&u).Error)
	token, _, _, _ := tm.IssuePair(u.ID)
...
func TestIntegration_UserFollow_Toggle(t *testing.T) {
	api, r, tm := newTestAPI(t)
	u1 := user.User{Username: "f1", PasswordHash: "hash", Nickname: "F1", CoinBalanceTenths: 100}
	u2 := user.User{Username: "f2", PasswordHash: "hash", Nickname: "F2", CoinBalanceTenths: 100}
	require.NoError(t, api.DB.Create(&u1).Error)
	require.NoError(t, api.DB.Create(&u2).Error)
	token, _, _, _ := tm.IssuePair(u1.ID)
...
func TestIntegration_PostSearchHistory(t *testing.T) {
	api, r, tm := newTestAPI(t)
	u := user.User{Username: "psh", PasswordHash: "hash", Nickname: "PSH", CoinBalanceTenths: 100}
	require.NoError(t, api.DB.Create(&u).Error)
	token, _, _, _ := tm.IssuePair(u.ID)
