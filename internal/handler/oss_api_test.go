...
func TestOSSUploadPaths(t *testing.T) {
	...
	_, err = fw.Write([]byte{0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 'J', 'F', 'I', 'F'})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = fw2.Write([]byte{0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 'J', 'F', 'I', 'F'})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = fw3.Write([]byte{0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 'J', 'F', 'I', 'F'})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = fw4.Write([]byte{0x89, 'P', 'N', 'G', 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D, 'I', 'H', 'D', 'R'})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = fw5.Write([]byte{0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 'J', 'F', 'I', 'F'})
	require.NoError(t, err)
