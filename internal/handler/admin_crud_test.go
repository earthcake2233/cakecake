...
func newAdminCRUDAPI(t *testing.T) (*API, *gin.Engine, *jwttoken.Manager) {
	...
	api := &API{Dependencies: &Dependencies{Cfg: cfg, DB: db, Redis: rdb, JWT: jm, Hub: ws.NewHub(), Log: zap.NewNop(), Play: nil, StorageSvc: storage.NewStorageService(cfg, nil, zap.NewNop()), VideoSvc: vsvc.NewVideoService(db, rdb, zap.NewNop(), nil, nil, nil), BannerSvc: banner.NewBannerService(db)}}
	r := gin.New()
