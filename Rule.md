...
| 编号         | 规则                                               | 说明                                                                                                                                                                                                                            |
| :----------- | :------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **R-DOC-1**  | **所有中英配对 .md 必须同步更新**                   | 项目中所有存在 `_EN.md` 配对的中文 `.md` 文件（包括 Rule、Skill、SPEC、DEPLOY、docs/*、cakecake-vue/**/README 等），修改任一方时必须同步更新另一方，确保内容、表格、代码块、链接保持一致，仅语言不同。提交前必须运行 `python scripts/check_en_sync.py --check-sync` 验证（章节结构与代码块级）。 |
...
| **R-VERIFY-5** | **提交前必须向作者展示改动并获确认（一次性确认门）** | 任何代码/文档改动在 `git commit` 之前，必须先向项目作者（earthcake）展示 `git status --short`（文件清单）与 `git diff`（关键内容），经作者明确确认后方可提交。提交时只 `git add` 本次改动的文件路径，禁止使用 `git add -A` 或 `git add .`（避免把作者未确认的改动或意外删除扫入提交）；commit 前必须再次核对暂存清单与已展示内容一致。确认门设在 commit 前一次完成，不要求 add 单独确认。禁止在未展示、未获确认的情况下直接 commit。 |
