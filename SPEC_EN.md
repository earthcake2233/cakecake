...
## cakecake v1.0 Design Specification (SPEC)
...
| Modify avatar         | `POST /api/v1/users/me/avatar`, `multipart/form-data`, field name **`avatar`**. Formats: **JPEG, PNG, GIF, BMP, WEBP** only; single file **≤ 5 MB** (Rule **R-BIZ-8**; validation flow same as Skill **S-005** cover extension/size logic, size cap 5MB). Uploads to OSS Bucket `cakecake`, key **`avatars/{user_id}.{ext}`** (see NF-7). Success returns `data.avatar_url`. Returns `50000` if OSS is not configured. |
...
| Default cover   | During transcode, if no custom cover was submitted, the system extracts the video's 1-second frame as the default cover and uploads it to OSS Bucket `cakecake` at `covers/{video_id}.jpg`.                                                                                                     |
| Custom cover    | Users can submit a custom cover with the upload (F2 `cover` field). After publishing, a cover-update endpoint can replace it. Requirements: - Formats: JPEG, PNG, GIF, BMP, WEBP only - Size ≤ 10 MB - Uploaded to OSS Bucket `cakecake` at `covers/{video_id}.{ext}`, replacing the old cover URL |
...
| Requirement  | Specification                                                                                                     |
| :----------- | :---------------------------------------------------------------------------------------------------------------- |
| Play source  | The frontend player plays the transcoded H.264 MP4 stored in Aliyun OSS Bucket `cakecake` under `videos/`, via its OSS access URL. |
