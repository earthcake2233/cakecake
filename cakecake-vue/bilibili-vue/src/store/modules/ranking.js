...
const mutations = {
  SET_RANK_ALL: (state, data) => {
    ...
    let list = raw.list;
    if (list && typeof list === "object" && !Array.isArray(list) && list.data) {
      list = list.data;
    }
...
const actions = {
  setRankData({ commit, state }, { rid, arc_type, day }) {
    if (minibiliApiOn() && state.firstMenuActive !== 0) {
      commit("SET_RANK_ALL", { note: "", list: [], comingSoon: true });
      commit("SET_LOADING");
      return;
    }
    let _type;
    ...
    function getRankData() {
      getRanking(_type, rid, arc_type, day)
        .then(res => {
          commit("SET_RANK_ALL", (res && res.data) || res);
          commit("SET_LOADING");
        })
        .catch(() => {
          commit("SET_RANK_ALL", { note: "", list: [] });
          commit("SET_LOADING");
        });
    }
  }
