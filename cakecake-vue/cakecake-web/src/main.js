...
import { registerCakecakeSessionInvalidate } from "./utils/cakecakeAuthSync";
import {
  getAccessToken,
  getRefreshToken
...
if (cakecakeEnv) {
  ...
  if (localStorage.getItem("signIn") === "1") {
    ...
    if (hasRefresh && needRefresh) {
      void refreshCakecakeAccessToken().then(ok => {
        if (ok) {
          void store.dispatch("login/refreshCakecakeMe");
        }
      });
