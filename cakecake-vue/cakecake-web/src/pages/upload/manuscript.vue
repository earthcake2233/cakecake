  ...
  <CreatorShell>
    <div class="mm-wrap">
      ...
      <template v-if="mainTab === 'video'">
        ...
        <p v-if="isCakecakeMode && videosFetchError" class="mm-video-error">
          {{ videosFetchError }}
        </p>
        <div
          v-if="isCakecakeMode && videosLoading && !videos.length"
      ...
      <template v-else>
        ...
        <template v-if="articleSubTab === 'article'">
          ...
          <div
            v-if="isCakecakeMode && articlesLoading && !displayArticles.length"
            class="mm-video-loading"
          >
            加载中…
          </div>
        ...
        <template v-else-if="articleSubTab === 'draft'">
          <div
            v-if="isCakecakeMode && articlesLoading && !draftArticles.length"
            class="mm-video-loading"
          >
            加载中…
          </div>
...
export default {
  ...
  computed: {
    ...
    displayVideos() {
      return this.isCakecakeMode ? this.videos : this.filteredVideos;
    },
  ...
  watch: {
    "$route.query"() {
      ...
      if (this.isCakecakeMode && this.mainTab === "video") {
        void this.reloadMyVideos();
      }
    },
    mainTab(val) {
      if (val === "video" && this.isCakecakeMode) {
        void this.reloadMyVideos();
      }
      if (val === "article" && this.isCakecakeMode) {
    ...
    articleSubTab(val) {
      if (!this.isCakecakeMode || this.mainTab !== "article") return;
  ...
  methods: {
    ...
    setArticleKindFilter(val) {
      if (this.articleKindFilter === val) {
        if (this.isCakecakeMode) void this.reloadMyArticles();
        return;
      }
    ...
    articleDisplayRoute(item) {
      ...
      if (item.kind === "moment") {
        if (item.status === "passed") {
          return cakecakeDynamicReadRoute(item.id);
        }
        return cakecakeUserSpaceDynamicRoute(this.meUserId);
      }
      if (item.status === "passed") {
        return cakecakeArticleReadRoute(item.id);
      }
      if (this.isCakecakeMode) {
    ...
    async refreshArticleStatusCounts() {
      if (!this.isCakecakeMode || !getAccessToken()) return;
      try {
    ...
    onArticleEdit(item) {
      ...
      if (this.isCakecakeMode && item.kind === "moment") {
        const route = cakecakeDynamicReadRoute(item.id, { edit: "1" });
        if (route) {
      ...
      if (this.isCakecakeMode && item.kind === "column") {
    ...
    videoPlayRoute(item) {
      ...
      return cakecakeVideoPlayRoute(item && item.id);
    },
    onMoreMenu(key, item) {
      if (this.moreLeaveTimer) {
        clearTimeout(this.moreLeaveTimer);
    ...
    setVideoStatusFilter(st) {
      if (this.statusFilter === st) {
        if (this.isCakecakeMode) void this.reloadMyVideos();
        return;
      }
      this.statusFilter = st;
      this.videoPage = 1;
