...
export default {
  ...
  computed: {
    ...
    editReplaceBlocked() {
      ...
      return this.isCakecakeApiEnv && this.isEditMode && !this.canReplaceVideoInEdit;
    },
    ...
    /** cakecake 发布且正在提交：用 XHR 上传进度；不确定态仅在上传阶段 */
    ...
    /** 进度条宽度：默认本地载入；cakecake 上传中改为上传进度 */
    ...
    /** 发布页：cakecake 走真实上传时的主按钮文案（R-FE-6 loading） */
    publishPrimaryLabel() {
      ...
      if (this.isEditMode && this.isCakecakeApiEnv) {
      ...
      if (this.isCakecakeApiEnv && this.isPublishMode && this.uploadProgress > 0) {
    ...
    videoFileStatusText() {
      if (this.isCakecakeApiEnv && this.isEditMode) {
        const st = String(this.editVideoStatus || "");
      ...
      if (this.isCakecakeApiEnv && this.isPublishMode) {
        if (this.isMetadataOnlyPublish) {
    ...
    shouldConfirmLeave() {
      ...
      if (
        ...
        (this.isEditMode && this.isCakecakeApiEnv)
      ) {
    ...
    footActionsDisabled() {
      return (
        this.uploadSubmitting ||
        (this.isCakecakeApiEnv && this.isEditMode && this.editVideoLoading)
      );
    },
    videoUploadDisabled() {
  ...
  methods: {
    ...
    bootstrap() {
      ...
      if (this.isEditMode) {
        if (this.isCakecakeApiEnv) {
          void this.bootstrapEditCakecake();
          return;
        }
    ...
    onSubmit() {
      if (this.isCakecakeApiEnv && this.isDraftEditMode) {
        void this.submitCakecakePublishDraft();
        return;
      }
      if (this.isCakecakeApiEnv && this.isPublishMode) {
        if (this.isMetadataOnlyPublish) {
    ...
    async submitCakecakePublishDraft() {
      ...
      try {
        if (this.editDirty || this.localVideoFile) {
          await this.submitCakecakeDraftSave({
            redirect: false,
    ...
    async submitCakecakeReplaceMedia() {
      if (this.uploadSubmitting) return;
      const id = this.editVideoNumericId;
    ...
    async submitCakecakeEdit() {
      ...
      if (this.isRejectedOrFailedEditMode && this.localVideoFile) {
        void this.submitCakecakeReplaceMedia();
        return;
      }
      const title = (this.form.title || "").trim();
      const titleLen = Array.from(title).length;
