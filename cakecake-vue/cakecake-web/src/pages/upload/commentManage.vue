...
import MbReplyComposerInner from "@/pages/cakecake/MbReplyComposerInner.vue";
...
export default {
  ...
  computed: {
    ...
    emptyText() {
      if (!this.isCakecakeMode) return "请登录后查看";
  ...
  watch: {
    ...
    searchQ(val) {
      ...
      if (!q && this.searchApplied) {
        this.searchApplied = "";
        if (this.showListArea && this.isCakecakeMode && getAccessToken()) {
          this.page = 1;
          void this.fetchList();
        }
      }
  ...
  methods: {
    ...
    setPrimaryTab(tab) {
      if (this.primaryTab === tab) {
        if (tab === "visible" && this.showListArea && this.isCakecakeMode && getAccessToken()) {
    ...
    userSpaceTo(row) {
      ...
      return cakecakeUserSpaceRoute(row.user_id);
    },
    rowMedia(row) {
      if (!row) return null;
    ...
    articleLink(row) {
      ...
      return cakecakeArticleReadRoute(row.article_id);
    },
    dynamicLink(row) {
      if (!row || !row.dynamic_id) return null;
      return cakecakeDynamicReadRoute(row.dynamic_id);
    },
    contentLink(row) {
