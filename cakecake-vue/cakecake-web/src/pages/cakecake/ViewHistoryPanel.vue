    ...
    <header class="mb-pc-history__hd">
      ...
      <div class="mb-pc-history__actions">
        ...
        <button
          ...
          :disabled="!isCakecakeMode || settingsLoading"
          @click="onTogglePause"
        >
        ...
        <button
          type="button"
          class="mb-pc-history__btn"
          :disabled="!isCakecakeMode || !items.length || clearing"
          @click="onClearAll"
        >
          清空历史
        </button>
  ...
  methods: {
    ...
    contentRoute(row) {
      if (this.isArticleRow(row)) {
        return (
          cakecakeArticleReadRoute(row.article_id) || {
      ...
      return (
        cakecakeVideoPlayRoute(row.video_id) || {
          name: "video",
          params: { aid: "0" }
        }
      );
    ...
    uploaderRoute(row) {
      return cakecakeUserSpaceRoute(row.uploader_id);
    },
    categoryLabel(row) {
    ...
    async onTogglePause() {
      if (!this.isCakecakeMode || this.settingsLoading) {
        return;
      }
      if (!this.paused) {
    ...
    async onClearAll() {
      if (!this.isCakecakeMode || this.clearing || !this.items.length) {
        return;
      }
      const ok = await this.confirmHistAction(
        "清空之后就什么都没有了哦~",
    ...
    async onDelete(row) {
      if (!this.isCakecakeMode || !row || this.deletingKey) {
        return;
      }
