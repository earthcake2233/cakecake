      ...
      <div class="mb-space__body">
        ...
        >
                  ...
                  <ul v-else :class="spaceVideoGridClass">
                    ...
                    >
                      <router-link
                        ...
                        :to="cakecakeVideoPlayRoute(v.id)"
                      >
                        <div
              ...
              <template v-else>
                ...
                >
                  ...
                  <ul v-else class="mb-space__article-list">
                    ...
                    >
                      <router-link
                        v-if="cakecakeArticleReadRoute(art.id)"
                        class="mb-space__article-link"
                        :to="cakecakeArticleReadRoute(art.id)"
                      >
          ...
          aria-label="更多"
                      ...
                      <router-link
                        v-if="cakecakeDynamicReadRoute(row.post.id)"
                        :to="cakecakeDynamicReadRoute(row.post.id)"
          ...
          aria-label="更多"
                      ...
                      <div class="mb-space__dyn-video-info">
                        <router-link
                          v-if="cakecakeArticleReadRoute(row.article.id)"
                          class="mb-space__dyn-vbox mb-space__dyn-vbox--article"
                          :to="cakecakeArticleReadRoute(row.article.id)"
                        >
          ...
          aria-label="更多"
                      ...
                      <div class="mb-space__dyn-video-info">
                        <router-link
                          ...
                          :to="cakecakeVideoPlayRoute(row.video.id)"
                        >
                          <div class="mb-space__dyn-vbox-l">
                            <img
                              class="mb-space__dyn-vcover"
            ...
            <div class="mb-space__collect-main">
              ...
              <template v-else-if="collectSideNav === 'articleFav'">
                ...
                >
                  ...
                  >
                    <router-link
                      v-if="!art.unavailable && cakecakeArticleReadRoute(art.id)"
                      class="mb-space__vcell-link"
                      :to="cakecakeArticleReadRoute(art.id)"
                    >
          ...
          <section class="mb-space__card mb-space__card--profile">
            <div class="mb-space__card-title-row">
              ...
              <router-link
                ...
                :to="{ path: '/cakecake/account', query: { tab: 'info' } }"
              >
                编辑
                <span class="mb-space__creator-chev" aria-hidden="true">›</span>
              </router-link>
...
import MbStationDialog from "@/components/cakecake/MbStationDialog.vue";
import MbSpaceHeaderActions from "@/components/cakecake/MbSpaceHeaderActions.vue";
import MbSpacePerspective from "@/components/cakecake/MbSpacePerspective.vue";
import MbSpacePerspectivePicker from "@/components/cakecake/MbSpacePerspectivePicker.vue";
import MbCollectVideoFolderTransferDialog from "@/components/cakecake/MbCollectVideoFolderTransferDialog.vue";
import MbCollectFolderCreateDialog from "@/components/cakecake/MbCollectFolderCreateDialog.vue";
import CakecakeCommentsLive from "./CakecakeCommentsLive.vue";
import MbDynVideoCommentPanel from "@/components/cakecake/MbDynVideoCommentPanel.vue";
import {
  MB_COMMENT_CURATED_LABEL,
  MB_COMMENT_CURATED_PLACEHOLDER
} from "@/constants/cakecakeComments";
import {
  mbGetUserPublic,
...
export default {
  ...
  computed: {
    ...
    /**
     * 侧栏展示用：UP 主 UID、生日等
     * 供 CakecakeCommentsLive 的 videoAuthorId 使用
     */
    ...
    /** 优先用 profile，否则回退 Vuex cakecakeMe */
    spaceGenderKey() {
      const p =
      ...
      if (
        ...
        this.isOwnSpace &&
        this.cakecakeMe &&
        this.cakecakeMe.gender != null
      ) {
        g = String(this.cakecakeMe.gender).trim();
      }
    ...
    /**
     * 生日优先 GET /space/:id 的 birthday，否则 Vuex cakecakeMe
     * 与账号资料页 profile 一致
     */
    asideBirthText() {
      ...
      if (
        ...
        const s = format(pickRaw(this.cakecakeMe));
        if (s) {
          return s;
        }
      }
  ...
  methods: {
    ...
    onCollectPlayAll() {
      ...
      const first = list[0];
      this.$router
        .push(cakecakeVideoPlayRoute(first.id))
        .catch(() => {});
    },
    ...
    syncCollectSideRoute(side) {
      const uid = this.userIdNum;
      if (!uid || this.$route.name !== "cakecakeUserSpace") return;
    ...
    selectCollectLater() {
      if (this.mbLoggedIn) {
        this.$router.push({ name: "cakecakeWatchLater" }).catch(() => {});
        return;
      }
    ...
    applySpaceRouteQuery(route) {
      ...
      if (q.nav === "collect" && q.side === "later") {
        this.$router.replace({ name: "cakecakeWatchLater" }).catch(() => {});
        return;
      }
      if (r.name !== "cakecakeUserSpace") {
        return;
      }
      if (q.nav === "dynamic") {
    ...
    syncSpaceRouteForNav(navKey, { usePush = false, side = null } = {}) {
      const uid = this.userIdNum;
      if (!uid || this.$route.name !== "cakecakeUserSpace") {
    ...
    dynRowSearchRoute(row) {
      ...
      if (row.kind === "video") {
        return this.cakecakeVideoPlayRoute(row.video.id);
      }
      if (row.kind === "article") {
        return this.cakecakeArticleReadRoute(row.article.id);
      }
      if (row.kind === "image") {
        return this.cakecakeDynamicReadRoute(row.post.id);
      }
      return null;
    },
    ...
    dynPinsStorageKey() {
      return `cakecake_space_dyn_pins_${this.userIdNum}`;
    },
    loadDynPins() {
    ...
    persistImageDynamics() {
      ...
      const key = `cakecake_space_dyn_img_${this.userIdNum}`;
      try {
    ...
    openRelations(tab) {
      ...
      const extra =
        this.isPerspectivePreview && this.spacePerspective !== "self"
          ...
          : {};
      const r = cakecakeUserSpaceRelationsRoute(this.userIdNum, t, extra);
      if (!r) {
        return;
      }
      if (this.profileLoading || !this.profile) {
