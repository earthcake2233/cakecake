      ...
      <div v-if="!token" class="msg-login-hint bili-wrapper">
        <div class="msg-login-card">
          <p class="msg-login-card__crumbs">
            ...
            <router-link class="msg-aside__crumb" to="/cakecake/account">
              个人中心
            </router-link>
          </p>
          <p>
...
import MbDmChatPanel from "@/components/cakecake/MbDmChatPanel.vue";
...
export default {
  ...
  computed: {
    ...
    meReplyFace() {
      const me = this.$store.state.login.cakecakeMe;
  ...
  methods: {
    openLoginModal() {
      openCakecakeLoginModal({
        tab: 0,
        redirect: "/cakecake/messages?cat=my_message"
      });
    },
    ...
    pushArticleWithComment(row) {
      ...
      const cid = Number(row.reply_comment_id) || 0;
      const query = cid > 0 ? { mb_cid: String(cid) } : {};
      const base = cakecakeArticleReadRoute(aid);
      const resolved = this.$router.resolve(
        base ? { ...base, query } : { path: `/cakecake/article/${aid}`, query }
      );
      const href = resolved.href;
      const url =
        href && /^https?:\/\//i.test(href)
