<template>
  <div class="mb-page bili-wrapper">
    <div class="mb-card">
      ...
      <p v-if="!isCakecakeEnv" class="mb-card__warn">
...
import { mbRegisterThenLogin } from "@/api/cakecake";
import {
...
export default {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      const passErr = validateCakecakeRegisterPassword(this.password);
      if (passErr) {
      ...
      try {
        ...
        await this.$store.dispatch("login/refreshCakecakeMe").catch(() => {});
        this.$router.replace("/");
      } catch (e) {
