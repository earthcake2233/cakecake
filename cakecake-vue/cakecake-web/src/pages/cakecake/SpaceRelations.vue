...
import MbSpaceChrome from "@/components/cakecake/MbSpaceChrome.vue";
import MbFollowGroupCreateDialog from "@/components/cakecake/MbFollowGroupCreateDialog.vue";
import MbRelFollowTag from "@/components/cakecake/MbRelFollowTag.vue";
...
export default {
  ...
  computed: {
    ...
    spaceRoute() {
      const r = cakecakeUserSpaceRoute(this.userIdNum);
      return r || { name: "home" };
    },
    isRealSpaceOwner() {
  ...
  methods: {
    userSpaceRoute(userId) {
      const r = cakecakeUserSpaceRoute(userId);
    ...
    onChromeNav(key) {
      const base = cakecakeUserSpaceRoute(this.userIdNum);
      if (!base) return;
      if (key === "collect") {
    ...
    replaceRelationsTab(tab) {
      ...
      const r = cakecakeUserSpaceRelationsRoute(
        this.userIdNum,
        t,
    ...
    async loadList() {
      ...
      try {
        ...
        const code = e && e.cakecakeApiCode;
        if (
          code === 40300 ||
          msg.includes("无权限") ||
          msg.includes("403") ||
