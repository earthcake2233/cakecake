          ...
          <div class="mb-pc__cols">
            ...
            <main class="mb-pc__main">
                    ...
                    <div class="mb-pc__profile-meta">
                      ...
                      <div class="mb-pc__profile-lv-row">
                        ...
                        <div class="mb-pc__profile-actions">
                          ...
                          <router-link
                            v-if="isCakecakeMode && cakecakeMe && cakecakeMe.user_id"
                            class="mb-pc__hd-act"
                            :to="cakecakeSpaceLinkTo"
                          >
                            个人空间
                  ...
                  <div class="mb-pc-myinfo__form">
                    ...
                    <div class="mb-pc-myinfo__actions">
                      <button
                        type="button"
                        class="mb-pc-myinfo__save"
                        :disabled="securityPwdSaving || !isCakecakeMode"
                    ...
                    <div
                      v-if="
                        isCakecakeMode &&
                          (!cakecakeMe || !cakecakeMe.pending_deletion)
                      "
              ...
              <PersonalCenterCoin
                ...
                :is-cakecake-mode="isCakecakeMode"
              />
...
import MbAvatarCropDialog from "@/components/cakecake/MbAvatarCropDialog.vue";
import {
  mbGetMe,
  mbGetMeDailyRewards,
  mbPostMeAvatar,
...
import { cakecakeErrorMessage, validateCakecakeRegisterPassword } from "@/utils/cakecakeAuthRules";
import { resolveUserAvatarUrl } from "@/utils/imageCacheBust";
import { cakecakeUserSpaceRoute } from "@/utils/cakecakeRoutes";
import {
  getAccessToken,
...
export default {
  ...
  computed: {
    ...
    cakecakeSpaceLinkTo() {
      if (!this.isCakecakeMode || !this.cakecakeMe || this.cakecakeMe.user_id == null) {
        return { path: "/" };
      }
      const r = cakecakeUserSpaceRoute(this.cakecakeMe.user_id);
      return r || { path: "/" };
    },
    profileStorageKey() {
    ...
    displayCakeId() {
      if (this.isCakecakeMode && this.cakecakeMe && this.cakecakeMe.cake_id) {
        return this.cakecakeMe.cake_id;
      }
      const p = this.proInfo;
      if (p && typeof p === "object" && !Array.isArray(p) && p.mid != null) {
    ...
    displayNickname() {
      if (this.isCakecakeMode && this.cakecakeMe) {
        const m = this.cakecakeMe;
        ...
        return getCakecakeDisplayName() || "用户";
      }
    ...
    avatarDisplayUrl() {
      ...
      if (this.isCakecakeMode && this.cakecakeMe) {
        const u = String(this.cakecakeMe.avatar_url || "").trim();
        if (u) {
    ...
    walletCoinDisplay() {
      if (this.isCakecakeMode && this.cakecakeMe) {
        return formatCoinBalance(this.cakecakeMe.coin_balance);
      }
      const p = this.proInfoObj;
    ...
    coinPageBalance() {
      if (this.isCakecakeMode && this.cakecakeMe) {
        return coinBalanceNumber(this.cakecakeMe.coin_balance);
      }
      const p = this.proInfoObj;
  ...
  watch: {
    "$route.query.tab"(v) {
      ...
      if (this.activeNavId === "coin") {
        void this.refreshCakecakeMe().then(() => {
  ...
  methods: {
    ...
    async onRevokeDeletionRequest() {
      if (!this.isCakecakeMode || this.revocationRevoking) {
        return;
      }
      ...
      try {
        ...
        this.securityFormError = cakecakeErrorMessage(e, "撤销失败");
      } finally {
    ...
    onBottomNavClick(id) {
      if (id === "space") {
        const to = this.cakecakeSpaceLinkTo;
        if (to && to.name === "cakecakeUserSpace") {
          void this.$router.push(to);
        } else if (this.isCakecakeMode) {
          void this.refreshCakecakeMe().then(() => {
            const next = this.cakecakeSpaceLinkTo;
            if (next && next.name === "cakecakeUserSpace") {
    ...
    selectPrimaryNav(id) {
      this.activeNavId = id;
      if (id === "home" && this.isCakecakeMode) {
        void this.loadDailyRewards();
      }
      if (id === "info") {
        this.hydrateMyInfoForm();
    ...
    profileDefaultsFromServer() {
      ...
      if (this.isCakecakeMode) {
        const nick = getCakecakeDisplayName() || "";
        return {
          nickname: nick,
          sign: "",
          gender: "secret",
    ...
    async onAvatarCropConfirm(file) {
      ...
      try {
        ...
        if (uploadedUrl && this.cakecakeMe) {
        ...
        this.openMbPcTipModal({
          title: "提示",
          message: cakecakeErrorMessage(e, "上传失败"),
          showSuccessImage: false,
          footerKind: "ok"
        });
      } finally {
    ...
    async onSubmitSecurityPassword() {
      ...
      const errNew = validateCakecakeRegisterPassword(newPwd);
      if (errNew) {
      ...
      try {
        ...
        this.securityFormError = cakecakeErrorMessage(e, "修改密码失败");
      } finally {
    ...
    async onMbPcTipModalDeregisterConfirm() {
      ...
      try {
        ...
        if (me) {
          this.$store.commit("login/SYNC_MINIBILI_ME", me);
        } else if (this.cakecakeMe && typeof this.cakecakeMe === "object") {
          this.$store.commit("login/SYNC_MINIBILI_ME", {
            ...this.cakecakeMe,
            pending_deletion: true,
            deletion_effective_at:
              res.deletion_effective_at ||
              this.cakecakeMe.deletion_effective_at ||
              null
          });
        }
        const effZh =
        ...
        this.closeMbPcTipModal();
        this.securityFormError = cakecakeErrorMessage(e, "提交失败");
      } finally {
    ...
    async onSaveMyInfo() {
      ...
      try {
        ...
        if (this.isCakecakeMode) {
          ...
          if (nickChanged) {
            const bal =
              this.cakecakeMe && typeof this.cakecakeMe.coin_balance === "number"
                ? this.cakecakeMe.coin_balance
                : 0;
            if (bal < 6) {
              this.openMbPcTipModal({
                title: "提示",
          ...
          await this.refreshCakecakeMe();
          setCakecakeDisplayName(payload.nickname);
          this.hydrateMyInfoForm();
          this.openMbPcTipModal({
            title: "提示",
            message: "已经成功更新你的资料",
        ...
        this.openMbPcTipModal({
          title: "提示",
          message: cakecakeErrorMessage(e, "保存失败"),
          showSuccessImage: false,
          footerKind: "ok"
        });
      } finally {
