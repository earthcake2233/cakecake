        ...
        <div class="video-body-stack">
          <div class="player-box-wrap notranslate" translate="no">
            ...
            <VideoPlayerBox
              ...
              :seek-to="_seekTime"
              :cakecake-video-id="mbNumericId != null ? mbNumericId : 0"
              :cakecake-danmaku-closed="!!(apiDetail && apiDetail.danmaku_closed)"
...
import CakecakeDanmakuFeed from "@/pages/cakecake/CakecakeDanmakuFeed.vue";
import MbUserHoverCard, {
  invalidateUserHoverProfileCache
} from "@/components/cakecake/MbUserHoverCard.vue";
...
export default {
  ...
  computed: {
    ...
    mbComposerAvatarSrc() {
      void this.cakecakeMe;
      void this.proInfo;
      if (this.isMb && this.cakecakeMe) {
    ...
    /** 播放页标签：cakecake 用详情接口 tags；本地演示保留占位 */
    videoTags() {
      if (!this.isMb) {
    ...
    /** 播放页分区面包屑（cakecake 详情 zone；无分区则不展示） */
    videoZoneCrumbs() {
    ...
    /** 交给 VideoPlayerBox 内层 <video>：cakecake 已发布稿件用 OSS MP4，否则走组件内演示片 */
    mbPlayerMediaSrc() {
      if (!this.isMb) return "";
    ...
    mbVideoAuthorRoute() {
      ...
      return cakecakeUserSpaceRoute(this.mbVideoAuthorId);
    },
    mbLoggedIn() {
      void this.$store.state.login.signIn;
      void this.$store.state.login.cakecakeMe;
  ...
  methods: {
    ...
    /** 连接 cakecake 后端时拉取稿件元数据，并通过 VideoPlayerBox 的 mediaSrc 播放 OSS */
    syncCakecakeDetail() {
      const on =
        import.meta.env.VITE_MINIBILI_API === "true" ||
        import.meta.env.VITE_MINIBILI_API === "1";
...
/* cakecake 评论嵌在原有 vd- 评论区顶栏下，收紧外框避免双重视觉边框 */
.vd-cmt-live-mount {
  overflow: visible;
}
