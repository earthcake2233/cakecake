...
const apiBase = isCakecake ? remoteTrim || "" : "";

const REFRESH_LOCK_KEY = "cakecake_refresh_lock";
...
export async function ensureFreshAccessToken() {
  ...
  return refreshCakecakeAccessToken();
}

/** 用 refresh_token 换新 access（单飞；多标签页通过 lock 协调） */
export async function refreshCakecakeAccessToken() {
  if (!isCakecake) {
    return false;
  }
  if (!getRefreshToken()) {
...
export function installCakecakeTokenAutoRefresh() {
  if (!isCakecake || typeof document === "undefined") {
    return;
  }
  const tick = () => {
    ...
    if (isAccessTokenExpired()) {
      void refreshCakecakeAccessToken();
    }
  };
