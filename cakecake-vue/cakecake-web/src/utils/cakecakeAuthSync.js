import { clearTokens, clearCakecakePostLoginRedirect } from "./authTokens";

let sessionInvalidateHandler = null;

/** 在 main 中注册：同步 Vuex（signIn / proInfo 等），避免 http 直接依赖 store 产生循环引用 */
export function registerCakecakeSessionInvalidate(handler) {
...
export function invalidateCakecakeSessionFromHttp() {
  const now = Date.now();
