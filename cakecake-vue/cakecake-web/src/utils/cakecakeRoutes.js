...
export const MINIBILI_COMPACT_HEADER_ROUTES = new Set([
  "notFound",
  "cakecakePersonalCenter",
  "cakecakeMessages",
  "cakecakeDynamics",
  "cakecakeArticleRead",
  "cakecakeDynamicRead",
  "cakecakeUserSpace",
  "cakecakeUserSpaceRelations"
]);

export function isCakecakeCompactHeaderRoute(routeName) {
  return MINIBILI_COMPACT_HEADER_ROUTES.has(routeName);
}

/** 个人空间 / 消息中心等：仅顶栏 nav-menu，不展示首页头图与分区导航 */
export function shouldShowCakecakeCompactHeader(route) {
  if (!route || !route.name) {
    return false;
  }
...
export function shouldShowHomeHeaderChrome(route) {
  return (
    !shouldShowCakecakeCompactHeader(route) && !isSearchLayoutRoute(route)
  );
}

/**
 * cakecake 前端路由片段（个人空间等），供顶栏 / 播放页 / 评论等复用。
 * @param {string|number|null|undefined} userId
 * @returns {{ name: string, params: { userId: string } }|null}
 */
export function cakecakeUserSpaceRoute(userId) {
  const raw = userId != null ? String(userId).trim() : "";
  const n = parseInt(raw, 10);
  if (!Number.isFinite(n) || n <= 0) {
    return null;
  }
  return { name: "cakecakeUserSpace", params: { userId: String(n) } };
}

/** 稍后再看独立页 */
export function cakecakeWatchLaterRoute() {
  return { name: "cakecakeWatchLater" };
}

/** 顶栏「动态」→ 独立关注流页 /cakecake/dynamics */
export function cakecakeDynamicsRoute() {
  return { name: "cakecakeDynamics" };
}

/** 个人空间内「动态」Tab */
export function cakecakeUserSpaceDynamicRoute(userId) {
  const base = cakecakeUserSpaceRoute(userId);
  if (!base) {
    return null;
  }
...
export function cakecakePersonalCenterRoute(tab) {
  const t = tab != null ? String(tab).trim() : "";
  if (!t) {
    return { name: "cakecakePersonalCenter" };
  }
  return { name: "cakecakePersonalCenter", query: { tab: t } };
}

/** 观看历史独立页（顶栏「历史」） */
export function cakecakeViewHistoryRoute() {
  return { name: "cakecakeViewHistory" };
}

/** 播放页路由（params.aid 为 BV{id}） */
export function cakecakeVideoPlayRoute(videoId) {
...
/** 专栏阅读页 */
export function cakecakeArticleReadRoute(articleId) {
  const n = parseInt(String(articleId ?? ""), 10);
  if (!Number.isFinite(n) || n <= 0) {
    return null;
  }
  return { name: "cakecakeArticleRead", params: { id: String(n) } };
}

/** 动态图文阅读页（复用 ArticleRead 布局）；query 如 { edit: "1" } 打开编辑弹窗 */
export function cakecakeDynamicReadRoute(dynamicId, query) {
  ...
  return { name: "cakecakeDynamicRead", params: { id: String(n) }, query: q };
}
 ...
 */
export function cakecakeUserSpaceCollectRoute(userId) {
  const base = cakecakeUserSpaceRoute(userId);
  if (!base) {
    return null;
  }
...
/** 个人空间 · 投稿 tab · 视频子栏 */
export function cakecakeUserSpaceContributeVideoRoute(userId) {
  const base = cakecakeUserSpaceRoute(userId);
  if (!base) {
    return null;
  }
 ...
 */
export function cakecakeUserSpaceRelationsRoute(
  userId,
  tab = "following",
  extraQuery = {}
) {
