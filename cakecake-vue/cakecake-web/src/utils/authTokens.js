/** cakecake JWT 本地存储（与主站 Vuex 登录弹窗独立） */
const K_ACCESS = "cakecake_access_token";
const K_REFRESH = "cakecake_refresh_token";
const K_UID = "cakecake_user_id";
const K_DISPLAY = "cakecake_display_name";
const K_POST_LOGIN = "cakecake_post_login_redirect";

/** 登录成功后要跳转的路径（仅 path，如 /upload、/cakecake/messages） */
export function setCakecakePostLoginRedirect(path) {
...
export function getCakecakeDisplayName() {
  return localStorage.getItem(K_DISPLAY) || "";
}

export function setCakecakeDisplayName(name) {
  if (name) {
