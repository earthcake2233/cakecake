...
export function validateCakecakeUsername(username) {
...
export function validateCakecakeRegisterPassword(password) {
  const p = password || "";
  if (p.length < 8) {
    return "密码至少 8 位";
  }
...
export function cakecakeErrorMessage(err, fallback = "请求失败") {
...
export function mapCakecakeLoginFailureMessage(err) {
  const d = err && err.response && err.response.data;
  const code = d && typeof d.code === "number" ? d.code : null;
  if (code === 40100 || code === 40101) {
    return "用户名或密码错误";
  }
  const raw = cakecakeErrorMessage(err, "");
  if (
    raw.includes("未登录") ||
    raw.includes("Token") ||
    raw.includes("token") ||
