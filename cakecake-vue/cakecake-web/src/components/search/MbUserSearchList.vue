<template>
  <div class="mb-user-search" :class="{ 'mb-user-search--compact': compact }">
    ...
    <ul v-if="items.length" class="mb-user-search__list">
      ...
      >
        <router-link
          :to="{ name: 'cakecakeUserSpace', params: { userId: user.mid } }"
...
import { cakecakeUserSpaceContributeVideoRoute } from "../../utils/cakecakeRoutes";
import { mbToggleUserFollow } from "../../api/cakecake";
  ...
  methods: {
    ...
    archiveRoute(arc) {
      if (arc.rtype === "article") {
        return { name: "cakecakeArticleRead", params: { id: arc.aid } };
      }
    ...
    async toggleFollow(user) {
      if (!getAccessToken()) {
        this.$router.push({ name: "cakecakeLogin" });
        return;
      }
