  ...
  <div class="login">
    ...
    <div class="login-form">
        ...
        <div class="login-content">
          <div class="user" :class="{ on: user !== '' }">
            <input
              ...
              type="text"
              value=""
              :placeholder="isCakecakeMode ? '用户名（支持中文）' : '你的手机号/邮箱'"
              maxlength="50"
              autocomplete="off"
              class="username"
            />
            <p v-if="!isCakecakeMode" class="error">{{ userError.errorText }}</p>
          </div>
          <div
          ...
          >
            ...
            <p v-if="!isCakecakeMode" class="error">{{ passError.errorText }}</p>
          </div>
        </div>
      ...
      <div class="register-user" v-else>
        <p v-if="isCakecakeMode" class="register-rule-hint">
        ...
        <div class="register-content">
          <div class="user" :class="{ on: user !== '' }">
            <input
              ...
              value=""
              :placeholder="
                isCakecakeMode ? MINIBILI_USERNAME_PLACEHOLDER : '昵称（例：哔哩哔哩）'
              "
              :maxlength="isCakecakeMode ? 32 : 50"
              autocomplete="off"
              class="username"
            />
            <p
          ...
          >
            <input
              ...
              :placeholder="
                isCakecakeMode ? '密码（至少 8 位）' : '密码（6-16个字符组成，区分大小写）'
              "
              class="userpassword userpassword--padded"
...
import { mbLogin, mbRegisterThenLogin } from "@/api/cakecake";
import { consumeCakecakePostLoginRedirect } from "@/utils/authTokens";
import {
...
export default {
  ...
  computed: {
    ...
    /** cakecake 登录：用户名合法 + 密码非空 */
    cakecakeLoginBtnReady() {
      const u = String(this.user || "").trim();
      const p = this.password || "";
      if (!u || !p) return false;
      return !validateCakecakeUsername(u);
    },
    ...
    /** cakecake 注册：用户名 + 密码均满足后端规则 */
    cakecakeRegisterBtnReady() {
      const u = String(this.user || "").trim();
      const p = this.password || "";
      if (!u || !p) return false;
      if (validateCakecakeUsername(u)) return false;
      if (validateCakecakeRegisterPassword(p)) return false;
      return true;
    },
    legacyLoginBtnReady() {
    ...
    loginSubmitLooksReady() {
      return this.isCakecakeMode
        ? this.cakecakeLoginBtnReady
        : this.legacyLoginBtnReady;
    },
    registerSubmitLooksReady() {
      return this.isCakecakeMode
        ? this.cakecakeRegisterBtnReady
  ...
  methods: {
    ...
    ...mapActions(["setSignIn", "setVipInfo", "refreshCakecakeMe"]),
    onTabClick(index) {
    ...
    onLogin() {
      ...
      if (this.isCakecakeMode) {
        ...
        const nameErr = validateCakecakeUsername(u);
        if (nameErr) {
        ...
        mbLogin(u, p)
          .then(() => {
            ...
            void this.refreshCakecakeMe().catch(() => {});
            const nextPath = consumeCakecakePostLoginRedirect();
            const target = nextPath || this.$route.path;
            this.$router.replace(target).catch(() => {});
          })
          .catch(e => {
            this.btnErrorText = mapCakecakeLoginFailureMessage(e);
          });
        return;
      }
    ...
    async onRegister() {
      ...
      if (this.isCakecakeMode) {
        ...
        const passErr = validateCakecakeRegisterPassword(p);
        if (passErr) {
        ...
        try {
          ...
          this.$store.commit("login/CLOSE_LOGIN_MODAL");
          this.setVipInfo().catch(() => {});
          await this.refreshCakecakeMe().catch(() => {});
          const nextPath = consumeCakecakePostLoginRedirect();
          const target = nextPath || this.$route.path;
          this.$router.replace(target).catch(() => {});
        } catch (e) {
          ElMessage.error(cakecakeErrorMessage(e, "注册失败"));
        }
        return;
      }
      ElMessage.info(
