<template>
  <div ref="rootRef" class="vp-root">
    ...
    >
      ...
      />
      <canvas
        v-show="cakecakeDanmakuOverlayVisible"
        ref="dmCanvasRef"
        class="vp-dm-canvas"
        aria-hidden="true"
      />
    ...
    <!-- 弹幕输入条：cakecake 未登录为游客条；已登录为完整输入 -->
    <div
      class="vp-input-bar"
      :class="{
        'vp-input-bar--guest':
          guestCakecakeDanmakuBar || upClosedCakecakeDanmakuBar
      }"
    >
      <template v-if="upClosedCakecakeDanmakuBar">
      ...
      <template v-else-if="guestCakecakeDanmakuBar">
        <div class="vp-input-guest-icons">
          <span class="vp-input-guest-ico-wrap">
            <img class="vp-input-bar-ico" :src="imgDanmuSend" alt="" />
          </span>
      ...
      <template v-else>
        ...
        <input
          ...
          :placeholder="
            cakecakeDanmakuClosed
              ? 'UP主已关闭弹幕'
              : '您可以在这里输入弹幕吐槽哦~'
          "
          :disabled="cakecakeDanmakuClosed"
          autocomplete="off"
...
import { mbPostDanmaku, mbPostMeDailyRewardWatch } from "@/api/cakecake";
...
/** cakecake 弹幕可选预设色（SPEC F5；任意合法 #RRGGBB 亦可） */
...
export default {
  ...
  props: {
    ...
    /** 非空时优先播放该地址（如 cakecake OSS MP4），否则用内置演示片 */
    mediaSrc: {
      type: String,
      default: ""
    },
    /** cakecake 稿件数字 ID；与 VITE_MINIBILI_API 同时生效时，主栏「发送」走 POST /videos/:id/danmaku（SPEC F5） */
    cakecakeVideoId: {
      type: Number,
      default: 0
    },
    ...
    /** cakecake：UP 关闭弹幕后禁止发送（与接口 40304 一致） */
    cakecakeDanmakuClosed: {
      type: Boolean,
      default: false
    }
  ...
  computed: {
    cakecakeDanmakuOverlayVisible() {
      return this.cakecakeDanmakuMode && this.danmuOn;
    },
    cakecakeDanmakuMode() {
      const on =
        import.meta.env.VITE_MINIBILI_API === "true" ||
        import.meta.env.VITE_MINIBILI_API === "1";
      return on && Number(this.cakecakeVideoId) > 0;
    },
    mbDanmakuLoggedIn() {
    ...
    /** cakecake 且未登录：游客提示条（与参考稿一致） */
    guestCakecakeDanmakuBar() {
      return this.cakecakeDanmakuMode && !this.mbDanmakuLoggedIn;
    },
    upClosedCakecakeDanmakuBar() {
      return this.cakecakeDanmakuMode && this.cakecakeDanmakuClosed;
    },
    ...
    danmuSendButtonDisabled() {
      if (!this.cakecakeDanmakuMode) return false;
      if (this.cakecakeDanmakuClosed) return true;
    ...
    danmuSendButtonLabel() {
      if (!this.cakecakeDanmakuMode) return "发送 >";
  ...
  watch: {
    ...
    cakecakeDanmakuMode() {
      if (!this.cakecakeDanmakuMode) this.dmResetEngine();
    },
    seekTo(t) {
      if (t > 0) {
        this._pendingSeek = t;
  ...
  methods: {
    ...
    dmOnPlaybackClock(prevT, currT) {
      if (!this.cakecakeDanmakuMode || !this.danmuOn) return;
    ...
    dmRafFrame() {
      const c = this.$refs.dmCanvasRef;
      if (!c || !this.cakecakeDanmakuOverlayVisible) return;
      const ctx = c.getContext("2d");
      if (!ctx) return;
    ...
    async sendCakecakeDanmaku() {
      if (!this.cakecakeDanmakuMode) return;
      if (this.danmuSending) return;
      if (this.cakecakeDanmakuClosed) {
      ...
      try {
        ...
        const row = await mbPostDanmaku(Number(this.cakecakeVideoId), {
          content: text,
          color,
