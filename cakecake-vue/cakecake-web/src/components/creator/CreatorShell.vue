...
import { getUserId, setCakecakePostLoginRedirect } from "@/utils/authTokens";
import {
...
export default {
  ...
  computed: {
    ...
    creatorSpaceTo() {
      ...
      return uid != null ? cakecakeUserSpaceRoute(uid) : null;
    },
    creatorMyUserId() {
      const m = this.cakecakeMe;
      if (m && m.user_id != null) {
  ...
  created() {
    if (cakecakeEnv) {
      void this.$store.dispatch("login/refreshCakecakeMe");
    }
  },
  mounted() {
