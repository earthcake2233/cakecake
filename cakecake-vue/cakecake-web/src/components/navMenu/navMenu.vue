    ...
    <div class="bili-wrapper">
      ...
      <div class="nav-con fr">
        <ul>
          ...
          >
            <router-link
              v-if="signIn == 1 && isCakecakeMode && cakecakeSpaceTo"
              class="t"
              :to="cakecakeSpaceTo"
            >
              <div class="i-face">
          ...
          <template v-if="signIn == 1">
            ...
            >
              <router-link
                v-if="isCakecakeMode"
                to="/cakecake/messages?cat=my_message"
                class="t"
                title="消息"
                ...
                <div class="im-list-box" v-show="messageShow">
                  <template v-for="item in messageNavItems" :key="item.cat">
                    <router-link
                      v-if="isCakecakeMode"
                      class="im-list"
                      :to="`/cakecake/messages?cat=${item.cat}`"
                      @click="messageShow = false"
                    >
                      {{ item.label }}
                      <div
            ...
            <li class="nav-item">
              <router-link
                v-if="isCakecakeMode && cakecakeDynamicsTo"
                class="t"
                :to="cakecakeDynamicsTo"
              >
                动态
              </router-link>
            ...
            <li class="nav-item">
              <router-link
                v-if="isCakecakeMode && cakecakeCollectTo"
                class="t"
                :to="cakecakeCollectTo"
              >
                收藏夹
              </router-link>
              <a v-else href="#" target="_blank" class="t">
...
import {
  ...
  cakecakeViewHistoryRoute,
  cakecakeUserSpaceCollectRoute,
...
export default {
  ...
  computed: {
    ...
    useSolidTopNav() {
      return !this.isHomeRoute || shouldShowCakecakeCompactHeader(this.$route);
    },
    isCakecakeMode() {
      return (
    ...
    cakecakeSpaceTo() {
      if (!this.isCakecakeMode || this.signIn != 1) {
        return null;
      }
      const p = this.navProfileRecord;
      if (!p || p.mid == null) {
      ...
      return cakecakeUserSpaceRoute(p.mid);
    },
    cakecakeCollectTo() {
      if (!this.isCakecakeMode || this.signIn != 1) {
        return null;
      ...
      return cakecakeUserSpaceCollectRoute(p.mid);
    },
    cakecakeDynamicsTo() {
      if (!this.isCakecakeMode || this.signIn != 1) {
        return null;
      }
      return cakecakeDynamicsRoute();
    },
    cakecakeHistoryTo() {
      if (!this.isCakecakeMode) {
        return "/";
      }
      return cakecakeViewHistoryRoute();
    },
    profileSpaceHref() {
      const p = this.navProfileRecord;
      if (p && p.mid != null) {
    ...
    /** 最右侧「投稿」：cakecake 未登录 → 弹主站登录窗 */
    cakecakeUploadOpensModal() {
      void this.$route.fullPath;
      return cakecakeUploadOpensLoginModal();
    },
    /** 已登录 cakecake 或非 Mini：router-link 目标 */
    uploadNavTo() {
      void this.$route.fullPath;
      return resolveCakecakeUploadNavTo();
    }
  },
  methods: {
    onMbUploadNavClick() {
  ...
  async created() {
    ...
    if (!login) {
      ...
      if (this.isCakecakeMode && String(login) === "1") {
