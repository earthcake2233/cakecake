...
const mutations = {
  ...
  SET_SEASON: (state, data) => {
    const row = state.allResult.result.media_bangumi[data.id];
    if (row) {
      row.season = data.result;
    }
  },
...
const actions = {
  setAllResult(
    { commit, state },
    { highlight, keyword, type = "all", sort = "", videoFilters = null }
  ) {
    getSearchResult(highlight, keyword, {
      type,
      sort,
      videoFilters
    }).then(res => {
  ...
  setSuggest({ commit, state }) {
    const term = String(state.searchWord || "").trim();
    if (!term) {
