...
import { mbGetMe } from "../../api/cakecake";
...
import { refreshCakecakeAccessToken } from "../../utils/cakecakeTokenRefresh";
...
function mapCakecakeMeToProInfo(me, avatarCacheBust = 0) {
...
const state = {
  ...
  signIn: "", //0为未登录，1为已登录
  proInfo: [], //个人信息
  /** cakecake GET /users/me 原始数据（个人中心表单等） */
  cakecakeMe: null,
...
const mutations = {
  ...
  /** cakecake：写入 users/me，并同步顶栏用的 proInfo */
  SYNC_MINIBILI_ME(state, me) {
    state.cakecakeMe = me == null ? null : me;
    state.proInfo = mapCakecakeMeToProInfo(me, state.avatarCacheBust);
  },
  BUMP_AVATAR_BUST(state) {
    state.avatarCacheBust = Date.now();
    if (state.cakecakeMe) {
      state.proInfo = mapCakecakeMeToProInfo(
        state.cakecakeMe,
...
const actions = {
  ...
  /** cakecake：拉取当前用户并写入 state（顶栏 + 个人中心共用） */
  async refreshCakecakeMe({ commit }) {
    if (!cakecakeEnv) {
      return;
    }
    if (!getAccessToken()) {
  ...
  //退出登录（含 cakecake：必须清 JWT，否则路由守卫仍放行创作中心）
  signOut({ commit }) {
    if (cakecakeEnv) {
      clearTokens();
      clearCakecakePostLoginRedirect();
