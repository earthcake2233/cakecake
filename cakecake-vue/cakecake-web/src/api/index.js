...
/** cakecake 后端：首页推荐等走真实 REST（需后端 CORS 与 VITE_REMOTE_API_BASE） */
const isCakecake =
...
/** B 站首页分区 rid → cakecake zone_parent（与 videoZones.js 一致） */
...
function cakecakeItemToArchive(v) {
  return {
    aid: v.id,
    title: v.title,
    pic: v.cover_url,
...
function fetchCakecakeZoneVideos({
  zoneParent,
  limit,
  sort,
  days,
...
//搜索框默认关键词（cakecake：热搜榜；其余：原站 mock）
export function getSearchDefaultWords() {
  if (isCakecake) {
    return http
      ...
      .catch(err => {
        console.warn("cakecake 热搜占位失败，回退本地热搜", err);
...
//搜索框搜索建议（cakecake：GET /api/v1/search/suggest）
...
function mapCakecakeRecommendItem(v) {
  return {
    aid: v.id,
    title: v.title,
    pic: v.cover_url,
  ...
  if (isCakecake) {
    const fetchPage = cursor => {
      ...
      return http.get("/api/v1/videos", { params }).then(body => {
        ...
        const data = body.data || {};
        const items = (data.items || []).map(mapCakecakeRecommendItem);
    ...
    return fetchPage("")
      ...
      .catch(err => {
        console.warn("cakecake 首页推荐池失败，回退本地 mock", err);
...
export function getRankingIndex(day) {
  if (isCakecake) {
    return http
      ...
      .catch(err => {
        console.warn("cakecake 首页列表失败，回退本地 mock", err);
...
export function getDynamicRegion(data) {
  ...
  if (isCakecake && zoneParent) {
    return fetchCakecakeZoneVideos({
      zoneParent,
      limit: data.ps,
      sort: "hot"
    })
      .then(({ items, zoneVideoCount }) => ({
        data: {
          archives: items.map(cakecakeItemToArchive),
          num: zoneVideoCount
        }
      }))
      .catch(err => {
        console.warn(
          `cakecake 分区「${zoneParent}」新动态失败，回退本地 mock`,
          err
        );
...
export function getNewlist(data) {
  ...
  if (isCakecake && zoneParent) {
    return fetchCakecakeZoneVideos({
      ...
      .catch(err => {
        console.warn(
          `cakecake 分区「${zoneParent}」最新投稿失败，回退本地 mock`,
          err
        );
...
export function getRankingRegion(data) {
  ...
  if (isCakecake && zoneParent) {
    return fetchCakecakeZoneVideos({
      ...
      .then(({ items }) => ({
        data: items.map(cakecakeItemToRank)
      }))
      .catch(err => {
        console.warn(
          `cakecake 分区「${zoneParent}」排行失败，回退本地 mock`,
          err
        );
...
export function getRanking(type, rid, arc_type = 0, day) {
  if (isCakecake) {
    ...
    return fetchCakecakeZoneVideos({
      ...
      .catch(err => {
        console.warn("cakecake 全站排行榜失败", err);
        return {
          data: {
            note: RANK_PAGE_NOTE,
            list: []
