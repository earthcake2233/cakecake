...
var mb;
beforeAll(async function() { mb = await import("@/api/cakecake"); });
...
describe("mbLogout",function(){it("clears localStorage",function(){
  localStorage.setItem("cakecake_access_token","tok");
  localStorage.setItem("cakecake_refresh_token","rt");
  mb.mbLogout();
  expect(localStorage.getItem("cakecake_access_token")).toBeNull();
  expect(localStorage.getItem("cakecake_refresh_token")).toBeNull();
});});
