...
vi.mock("@/utils/cakecakeTokenRefresh",()=>({isAccessTokenExpired:vi.fn(),refreshCakecakeAccessToken:vi.fn()}));
...
describe("router guards",()=>{
  ...
  it("refreshes token for requireCakecakeAuth",async()=>{
    var{isAdminLoggedIn}=await import("@/utils/adminAuth");
    isAdminLoggedIn.mockReturnValue(false);
    var{shouldRedirectVideoToNotFound}=await import("@/utils/notFoundRedirect");
    shouldRedirectVideoToNotFound.mockReturnValue(false);
    var{getAccessToken,getRefreshToken}=await import("@/utils/authTokens");
    var{isAccessTokenExpired,refreshCakecakeAccessToken}=await import("@/utils/cakecakeTokenRefresh");
    getAccessToken.mockReturnValue("expired");
    isAccessTokenExpired.mockReturnValue(true);
    getRefreshToken.mockReturnValue("rt");
    refreshCakecakeAccessToken.mockResolvedValue(true);
    var to={matched:[{meta:{requireCakecakeAuth:true}}]}; var next=vi.fn(); await guard(to,{},next);
    expect(refreshCakecakeAccessToken).toHaveBeenCalled(); expect(next).toHaveBeenCalledWith();
  });
  it("redirects home when requireCakecakeAuth fails",async()=>{
    var{isAdminLoggedIn}=await import("@/utils/adminAuth");
    isAdminLoggedIn.mockReturnValue(false);
    var{shouldRedirectVideoToNotFound}=await import("@/utils/notFoundRedirect");
    shouldRedirectVideoToNotFound.mockReturnValue(false);
    ...
    isAccessTokenExpired.mockReturnValue(true);
    getRefreshToken.mockReturnValue("rt");
    refreshCakecakeAccessToken.mockResolvedValue(false);
    var to={matched:[{meta:{requireCakecakeAuth:true}}]}; var next=vi.fn(); await guard(to,{},next);
