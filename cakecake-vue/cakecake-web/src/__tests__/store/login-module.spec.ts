...
describe("login state",()=>{
  ...
  it("cakecakeMe null",()=>expect(loginModule.state.cakecakeMe).toBeNull());
});

describe("login mutations",()=>{
  var{mutations}=loginModule;var s;
  beforeEach(()=>{s=JSON.parse(JSON.stringify(loginModule.state));s.cakecakeMe=null;s.avatarCacheBust=0;});
  ...
  it("SYNC_MINIBILI_ME",()=>{mutations.SYNC_MINIBILI_ME(s,{user_id:1,nickname:"T",coin_balance:100});expect(s.cakecakeMe).toBeTruthy();expect(s.proInfo.isLogin).toBe(true);expect(s.proInfo.money).toBe(100);});
  it("SYNC_MINIBILI_ME null",()=>{mutations.SYNC_MINIBILI_ME(s,null);expect(s.cakecakeMe).toBeNull();expect(s.proInfo).toEqual([]);});
