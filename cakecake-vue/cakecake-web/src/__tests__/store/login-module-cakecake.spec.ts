...
var mMB=vi.fn();
vi.mock("@/api/cakecake",()=>({mbGetMe:(...a)=>mMB(...a)}));
vi.mock("@/utils/authTokens",()=>({getAccessToken:vi.fn(),getRefreshToken:vi.fn(),clearTokens:vi.fn(),clearCakecakePostLoginRedirect:vi.fn()}));
vi.mock("@/utils/cakecakeTokenRefresh",()=>({refreshCakecakeAccessToken:vi.fn()}));
...
describe("login actions (cakecake env)",()=>{
  beforeEach(()=>{vi.clearAllMocks();});

  it("refreshCakecakeMe no access token clears",async()=>{
    ...
    await mod.actions.refreshCakecakeMe({commit:c});
    expect(c).toHaveBeenCalledWith("SYNC_MINIBILI_ME",null);
  });

  it("refreshCakecakeMe refreshes and fetches",async()=>{
    var at=await import("@/utils/authTokens");var tr=await import("@/utils/cakecakeTokenRefresh");
    at.getAccessToken.mockReturnValue(null);at.getRefreshToken.mockReturnValue("rt");
    tr.refreshCakecakeAccessToken.mockResolvedValue(true);
    mMB.mockResolvedValue({user_id:1,nickname:"Test"});
    var c=vi.fn();var{default:mod}=await import("@/store/modules/login");
    await mod.actions.refreshCakecakeMe({commit:c});
    expect(tr.refreshCakecakeAccessToken).toHaveBeenCalled();
    expect(c).toHaveBeenCalledWith("SYNC_MINIBILI_ME",{user_id:1,nickname:"Test"});
  });

  it("refreshCakecakeMe refresh fails clears",async()=>{
    var at=await import("@/utils/authTokens");var tr=await import("@/utils/cakecakeTokenRefresh");
    at.getAccessToken.mockReturnValue(null);at.getRefreshToken.mockReturnValue("rt");
    tr.refreshCakecakeAccessToken.mockResolvedValue(false);
    var c=vi.fn();var{default:mod}=await import("@/store/modules/login");
    await mod.actions.refreshCakecakeMe({commit:c});
    expect(c).toHaveBeenCalledWith("SYNC_MINIBILI_ME",null);
  });

  it("refreshCakecakeMe refresh fails sets signIn=0",async()=>{
    var at=await import("@/utils/authTokens");var tr=await import("@/utils/cakecakeTokenRefresh");
    at.getAccessToken.mockReturnValue(null);at.getRefreshToken.mockReturnValue("rt");
    tr.refreshCakecakeAccessToken.mockResolvedValue(false);
    var c=vi.fn();var{default:mod}=await import("@/store/modules/login");
  ...
  it("refreshCakecakeMe with access token fetches directly",async()=>{
  ...
  it("refreshCakecakeMe with no access and no refresh sets signIn=0",async()=>{
    var at=await import("@/utils/authTokens");at.getAccessToken.mockReturnValue(null);at.getRefreshToken.mockReturnValue(null);
    var c=vi.fn();var{default:mod}=await import("@/store/modules/login");
    await mod.actions.refreshCakecakeMe({commit:c});
    expect(c).toHaveBeenCalledWith("SET_SIGNIN",{signIn:"0"});
