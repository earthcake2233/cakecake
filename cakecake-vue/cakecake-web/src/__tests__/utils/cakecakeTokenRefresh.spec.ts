...
import {
  ...
  installCakecakeTokenAutoRefresh
...
describe("cakecakeTokenRefresh", () => {
  ...
  describe("refreshCakecakeAccessToken", () => {
    it("returns false without refresh token", async () => {
      expect(await refreshCakecakeAccessToken()).toBe(false);
    });
    it("calls refresh API and sets tokens on success", async () => {
      ...
      expect(await refreshCakecakeAccessToken()).toBe(true);
  ...
  describe("installCakecakeTokenAutoRefresh", () => {
    it("does not throw when document is defined", () => {
      expect(() => installCakecakeTokenAutoRefresh()).not.toThrow();
    });
  });
});
