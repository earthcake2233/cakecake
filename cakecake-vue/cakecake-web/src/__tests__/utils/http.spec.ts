...
vi.mock("@/utils/cakecakeTokenRefresh", function() {
  return { ensureFreshAccessToken: vi.fn(), isAccessTokenExpired: vi.fn(), refreshCakecakeAccessToken: vi.fn(), shouldAttemptTokenRefresh: vi.fn() };
});
vi.mock("@/utils/cakecakeAuthSync", function() { return { invalidateCakecakeSessionFromHttp: vi.fn() }; });
...
describe("request interceptor", function() {
  ...
  it("adds Bearer token", async function() {
    ...
    var tr=await import("@/utils/cakecakeTokenRefresh");tr.isAccessTokenExpired.mockReturnValue(false);
  ...
  it("auto-refreshes when token expired", async function() {
    ...
    var tr=await import("@/utils/cakecakeTokenRefresh");tr.isAccessTokenExpired.mockReturnValue(true);tr.ensureFreshAccessToken.mockResolvedValue();
  ...
  it("skips auth for auth API URLs", async function() {
    var at=await import("@/utils/authTokens");at.getRefreshToken.mockReturnValue("rt");
    var tr=await import("@/utils/cakecakeTokenRefresh");tr.isAccessTokenExpired.mockReturnValue(true);
...
describe("response success handler", function() {
  ...
  it("invalidates session when refresh fails", async function() {
    var tr=await import("@/utils/cakecakeTokenRefresh");tr.shouldAttemptTokenRefresh.mockReturnValue(true);tr.refreshCakecakeAccessToken.mockResolvedValue(false);
    var as=await import("@/utils/cakecakeAuthSync");
    await expect(resSuccess({data:{code:40100,msg:"err"},config:{url:"/api/users"}})).rejects.toThrow("err");
    expect(as.invalidateCakecakeSessionFromHttp).toHaveBeenCalled();
  });
});
...
describe("response error handler", function() {
  ...
  it("propagates cakecakeApiCode", async function() {
    try { await resError({response:{status:500,data:{msg:"fail"}},config:{url:"/api/v1"},cakecakeApiCode:50001}); }
    catch(e) { expect(e.cakecakeApiCode).toBe(50001); }
  });
});
